...
bool FastState::is_move_legal(const int color, const int vertex) const {
    return (vertex == FastBoard::PASS || vertex == FastBoard::RESIGN
               || (vertex != m_komove
                   && board.get_state_vt(vertex) == FastBoard::EMPTY
                   && !board.is_suicide(vertex, color)));
}

void FastState::play_move(const int vertex) {
    _play_move(board.m_tomove, vertex);
}

void FastState::_play_move(const int color, const int vertex) {
