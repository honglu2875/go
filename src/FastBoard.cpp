...
FastBoard::vertex_t FastBoard::get_state_vt(const int vertex) const {
...
void FastBoard::set_state_vt(const int vertex, const FastBoard::vertex_t content) {
    assert(vertex >= 0 && vertex < NUM_VERTICES);
    assert(vertex >= 0 && vertex < m_numvertices);
...
bool FastBoard::is_suicide(const int i, const int color) const {
    ...
    for (auto k = 0; k < 4; k++) {
        ...
        if (get_state_vt(ai) == color) {
            if (libs > 1) {
                // connecting to live group = not suicide
                return false;
            }
        } else if (get_state_vt(ai) == !color) {
            if (libs <= 1) {
...
void FastBoard::print_columns() {
    for (int i = 0; i < get_boardsize(); i++) {
        if (i < 25) {
            Utils::myprintf("%c ", (('a' + i < 'i') ? 'a' + i : 'a' + i + 1));
        } else {
            Utils::myprintf("%c ", (('A' + (i - 25) < 'I') ? 'A' + (i - 25)
                                                    : 'A' + (i - 25) + 1));
        }
    }
    Utils::myprintf("\n");
...
bool FastBoard::starpoint_vt(const int size, const int point) {
    int stars[3];
    int points[2];
    int hits = 0;
...
bool FastBoard::starpoint(const int size, const int x, const int y) {
    return starpoint_vt(size, y * size + x);
}
