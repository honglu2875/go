...
'''
    subprocess.run(ssh+[peer,'python3 -c '+shlex.quote(prepare)],check=True)
    subprocess.run(['rsync','-a','-e',shlex.join(ssh),str(source)+'/',peer+':'+str(target)+'/'],check=True)
    subprocess.run(['rsync','-a','-e',shlex.join(ssh),str(group),peer+':'+str(target.with_suffix('.group.json'))],check=True)
...
'''
    remote=json.loads(subprocess.check_output(ssh+[peer,'python3 -c '+shlex.quote(code)],text=True))
