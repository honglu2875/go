...
def main():
    ...
    version=hashlib.sha256(b'go-corpus19-v1\0'+native.read_bytes())
    ...
    run=root/'runs/expert19-v1';run.mkdir(parents=True,exist_ok=True)
    ...
    deployment=dict(root=str(root),environment=str(environment),contract=str(path),contract_id=contract_id,
                    run_id='expert19-v1',snapshot=key,prepared=time.time())
