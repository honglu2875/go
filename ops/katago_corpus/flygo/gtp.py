...
def vertex_to_action(vertex: str, size: int) -> int:
    """Convert GTP coordinates to the native top-left row-major action index."""
    if not 1 <= size <= len(_COLUMNS):
        raise ValueError('Board size outside GTP coordinate range')
    if vertex.lower() == 'pass':
        return size * size
    match = re.fullmatch(r'([A-HJ-Z])([1-9][0-9]*)', vertex.upper())
    if match is None:
        raise ValueError('Invalid GTP vertex: ' + vertex)
    col, row = _COLUMNS.index(match[1]), size - int(match[2])
    if not (0 <= col < size and 0 <= row < size):
        raise ValueError('GTP vertex outside the board: ' + vertex)
    return row * size + col


def action_to_vertex(action: int, size: int) -> str:
    if not 1 <= size <= len(_COLUMNS) or not 0 <= action <= size * size:
        raise ValueError('Action outside GTP coordinate range')
    if action == size * size:
        return 'pass'
    row, col = divmod(action, size)
    return _COLUMNS[col] + str(size - row)


def board_cells(text: str, size: int) -> str:
    """Parse all numbered rows of KataGo's showboard response."""
    rows = {}
    for line in text.splitlines():
        match = re.match(r'^\s*(\d+)\s+(.+)$', line)
        if match:
            cells = ''.join(c for c in match[2] if c in '.XO')
            if len(cells) == size:
                rows[int(match[1])] = cells
    if set(rows) != set(range(1, size + 1)):
        raise ValueError('Could not parse the complete KataGo board')
