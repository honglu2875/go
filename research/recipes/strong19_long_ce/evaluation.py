...
def totals(p,b,c,*,chunk_frames=32,axis_name=None):
    ...
    result={**policy_model.total_metrics(out['policy'],b,axis_name,stratify=True),
            **heads.value_totals(out['value'],b,axis_name=axis_name,stratify=True)}
    diagnostics=heads.signed_target_cross_entropy(out['value_logits'],b,axis_name=axis_name)
    result.update({'logit_'+k+'_sum':v*result['value_count'] for k,v in diagnostics.items()})
    return result
