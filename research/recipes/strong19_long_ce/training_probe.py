...
def overfit_observation(validation, training, *, patience=3, relative_margin=.005, metric='expert_kl'):
    ...
    best=min(range(len(rows)),key=lambda i:rows[i][0]['metrics'][metric])
    v0,t0=(r['metrics'][metric] for r in rows[best])
    recent=rows[-patience:]
    flagged=len(recent)==patience and all(v['turn']>rows[best][0]['turn']
        and v['metrics'][metric]>v0+relative_margin*max(abs(v0),1e-6)
        and t['metrics'][metric]<t0-relative_margin*max(abs(t0),1e-6) for v,t in recent)
    return dict(sustained=flagged,best_validation_turn=rows[best][0]['turn'],
                patience=patience,relative_margin=relative_margin,metric=metric,action='record only; preserve registered screen horizon')
