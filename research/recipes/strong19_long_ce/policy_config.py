...
def validate(c):
    ...
    if not fields <= set(c) or set(c)-fields-{'profile_decode', 'checkpoint_archive', 'checkpoint_temporary','checkpoint_temporary_uncompressed','draft_reference'} or c['schema_version']!=1 or c['kind']!='fixed_policy_learning':
    ...
    # This recipe studies new readouts with the already established auxiliary
    # objective. Parent checkpoints have different head schemas; the comparison
    # uses pinned audited learning curves rather than loading incompatible heads.
    if 'checkpoint_temporary' in c:
        if (c['checkpoint_temporary'] is not True or c['platform']!='tpu' or 'checkpoint_archive' in c
                or type(c['steps']) is not int or type(c['checkpoint_every']) is not int
                or c['steps']<1 or c['checkpoint_every']<1 or c['steps']%c['checkpoint_every']
                or c['steps']//c['checkpoint_every']>2):
            raise ValueError('Temporary storage admits at most two equally spaced checkpoints with explicit reservation')
    if 'checkpoint_temporary_uncompressed' in c and (c['checkpoint_temporary_uncompressed'] is not True or not c.get('checkpoint_temporary')):
        raise ValueError('Uncompressed checkpoints require explicit RAM staging')
    ...
    e=c['evaluation']
    if not set('games_per_bucket run_test'.split())<=set(e) or set(e)-set('games_per_bucket run_test training_probe_games'.split()) or type(e['run_test']) is not bool:
        raise ValueError('Invalid evaluation contract')
    integer(e,'games_per_bucket',1,200000)
    if 'training_probe_games' in e:integer(e,'training_probe_games',1,10000)
    return c
