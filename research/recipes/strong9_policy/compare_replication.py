"""Combine two paired first-pass auxiliary experiments, retaining both seeds."""
import argparse
import json
...
def main():
    ...
    for path,h in a.pair:
        ...
        if contrast['status']!='passed' or contrast['kind']!='first_pass_auxiliary_contrast':raise ValueError('Invalid spatial audit')
        ...
        pair={'seed':seed,'contrast':str(path),'contrast_sha256':h,'control':control,'candidate':candidate,
              ...
              'draft_utility_screen_passed':contrast['draft_utility_screen_passed'],
              'draft_metrics':{k:contrast[k] for k in ('parent_draft_kl','trained_draft_kl',
                  'relative_draft_kl_improvement','parent_draft_full_policy_overlap','trained_draft_full_policy_overlap',
                  'draft_full_policy_overlap_increase','parent_draft_full_argmax_agreement',
                  'trained_draft_full_argmax_agreement','trained_draft_decode_ms',
                  'trained_draft_over_full_neural_latency')}}
        pairs.append(pair)
    ...
    for role,key in [('control','control_snapshot'),('candidate','candidate_snapshot')]:
        ...
        for pair in pairs:
            ...
            if role=='candidate':c.pop('draft_reference',None)
            configs.append(c)
            hashes={n:v['sha256'] for n,v in m['files'].items() if n.startswith('packages/') or n in ('uv.lock','pyproject.toml','Cargo.lock')}
            names=NUMERICAL_FILES if role=='candidate' else tuple(n for n in NUMERICAL_FILES if n not in ('draft_model.py','draft_mask.py','profile_draft.py','qualify_draft_runtime.py','training_arithmetic.py','reference_draft.py'))
            hashes.update({'recipe/'+n:sha256(source/m['recipe']/n) for n in names})
    ...
    out={'status':'passed','kind':'two_seed_first_pass_auxiliary_replication','operator_snapshot':SOURCE.name,
        ...
        'each_seed_passed_draft_utility_screen':all(x['draft_utility_screen_passed'] for x in pairs),
    ...
    axes[0].set_ylabel('Validation KL');axes[1].set_ylabel('Auxiliary minus control KL')
    axes[1].axhline(0,color='black',linewidth=.8)
    for ax in axes:ax.set_xlabel('Optimizer updates');ax.grid(alpha=.2);ax.legend(fontsize=9)
    fig.suptitle('First-pass auxiliary supervision: both paired seeds')
    fig.savefig(a.output/'learning.png',dpi=160)
    fig.savefig(a.output/'learning.svg',metadata={'Date':None})
    plt.close(fig)
    lines=['The auxiliary-supervision replication retains both independent seeds. Configuration and numerical source change only by seed and the corresponding pinned parent checkpoint; actual game and D4 draws match within each pair.',
           '', '| Seed | Position exposures per arm | Control KL | Auxiliary KL | Relative KL improvement | Decode latency ratio |',
           '|---|---:|---:|---:|---:|---:|']
    for pair in pairs:
    ...
    lines+=['',f"Mean KL: control {control_mean:.6f}; auxiliary {candidate_mean:.6f}. Relative improvement of the mean: {out['relative_improvement_of_mean_kl']:.2%}.",
            ...
            '',f"Both seeds meet the separately registered draft-utility screen: {out['each_seed_passed_draft_utility_screen']}. This does not promote a weaker full policy as an architecture improvement.",
