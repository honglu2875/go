...
def run(args, c, report):
    ...
    def global_batch(batch):
        # Values remain available in the corpus for the subsequent joint study.
        # This registered stage optimizes and transfers policy targets only.
        return {k: jax.make_array_from_process_local_data(batched, v) for k, v in batch.items() if k!='values'}
    ...
    if c.get('profile_decode', False) and net['architecture']=='katago_nested_policy':
        import profile_cnn
        report['decode_profile']=profile_cnn.run(params,data,net,global_batch,world,args.output)
        print(json.dumps({'kind':'policy_decode_profile','host':host,**report['decode_profile']}),flush=True)
    ...
    def evaluate(split,*,draft=False,entries_by_bucket=None):
        ...
        family_weights=data.evaluation_family_weights([entry for bucket in buckets for entry in chosen_by_bucket[bucket]])
        batch_size = opt['games_per_host']
        for bucket in buckets:
            entries = chosen_by_bucket[bucket]
    ...
    if c.get('profile_decode',False) and net['architecture']=='katago_nested_policy':
        import profile_cnn
        trained_output=args.output/'trained_decode';trained_output.mkdir(exist_ok=False)
        report['trained_decode_profile']=profile_cnn.run(params,data,net,global_batch,world,trained_output)
        print(json.dumps({'kind':'trained_policy_decode_profile','host':host,**report['trained_decode_profile']}),flush=True)
    if net.get('first_pass_aux_weight',0):
