...
def run(args, c, report):
    ...
    runtime_config=optimizer_config['source_runtime']
    ...
    norms=jax.jit(optimizer.model_norms,out_shardings=replicated)
    baseline={k:float(x) for k,x in replica(norms(params)).items()}
    runtime=source_runtime.initialize(runtime_config,c['steps'],baseline)
    state=optimizer.initialize(params,optimizer_config,jax.device_put(source_runtime.vector(runtime['settings']),replicated))
    ...
    if args.resume is not None:
        ...
        restored,restored_state=optimizer_io.restore(saved['optimizer_metadata'],arrays,schema=expected_schema,
            configuration_sha256=config_sha,source_sha256=SOURCE.name,lookahead_config=runtime_config['lookahead'])
        ...
        runtime=saved['source_runtime']
        source_runtime.validate_state(runtime,runtime_config,c['steps'],positions=counters['expert_positions'])
        if runtime['turn']!=turn or not np.array_equal(restored_state['hyperparameters'],source_runtime.vector(runtime['settings'])):
            raise ValueError('Host schedule and device scalars disagree')
    ...
    def save():
        ...
        source_runtime.validate_state(runtime,runtime_config,c['steps'],positions=counters['expert_positions'])
        if runtime['turn']!=turn or not np.array_equal(arrays['hyperparameters'],source_runtime.vector(runtime['settings'])):
            raise ValueError('Saved runtime and dynamic settings disagree')
        ...
        saved = {'schema_version': 1, 'kind': 'visual_replicated_rank_state', 'snapshot_id': SOURCE.name,
                 ...
                 'optimizer_metadata':optimizer_metadata,'source_runtime':runtime,'validation_history':history,
    ...
    with (args.output / 'metrics.jsonl').open('x') as log:
        while turn < stop:
            runtime,before=source_runtime.before_step(runtime,runtime_config,c['steps'])
            if before['epoch_entry']:
                state=dict(state,hyperparameters=jax.device_put(source_runtime.vector(runtime['settings']),replicated))
            if before['subepoch_entry']:
                slow=lookahead.begin_subepoch(state['lookahead'])
                slow=dict(slow,counter=jax.device_put(np.asarray(0,np.int32),replicated))
                state=dict(state,lookahead=slow)
            start = time.perf_counter()
            ...
            pre_norms={k:float(v) for k,v in replica(norms(params)).items()} if before['is_print_batch'] else None
            ...
            runtime,after=source_runtime.after_step(runtime,runtime_config,c['steps'],
                positions=int(observed['positions']),pre_update_norms=pre_norms)
            if after['refreshed']:
                state=dict(state,hyperparameters=jax.device_put(source_runtime.vector(runtime['settings']),replicated))
            if after['epoch_end']:
                params,slow=lookahead.end_epoch(params,state['lookahead'],runtime_config['lookahead'])
                state=dict(state,lookahead=slow)
                jax.block_until_ready((params,state))
            observed.update(source_samples=runtime['samples'],source_refresh_count=runtime['refresh_count'],
                source_norm_snapshot=float(after['norm_snapshot']),source_epoch_flush=float(after['epoch_end']),
                source_pre_norm_input=pre_norms['input'] if pre_norms is not None else 0.,
                source_pre_norm_normal=pre_norms['normal'] if pre_norms is not None else 0.,
                source_subepoch_entry=float(before['subepoch_entry']),source_epoch_entry=float(before['epoch_entry']))
            ...
            if turn % c['eval_every'] == 0 or turn == c['steps'] or after['epoch_end']:
    ...
    report.update(status='passed', turn=turn, training_complete=turn == c['steps'],
                  ...
                  last_metrics=observed, source_runtime=runtime, segment_elapsed_seconds=time.perf_counter() - segment_start,
...
def entry(args):
    ...
    report = {'schema_version': 1, 'kind': c['kind'], 'snapshot_id': SOURCE.name, 'status': 'running',
              ...
              'training_purpose':c['training']['purpose'],'optimizer_family':'source_muon_aux_adam_lookahead',
              'batch_contract':'Actual globally live positions scale gradients and advance samples; schedule uses its separately declared reference-position batch. Qualification only; no fixed-batch reproduction claim.'}
