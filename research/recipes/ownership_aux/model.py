...
def apply(params,features,config,*,with_ownership=False):
    ...
    if with_ownership:
        return logits,value,conv(x,params['ownership']).astype(jnp.float32)[...,0]+params['ownership_bias']
    return logits,value
...
def losses(params,features,policies,outcomes,ownership,model_config,learner_config):
    logits,values,owner_logits=apply(params,features,model_config,with_ownership=True)
    ...
    return policy_loss+value_loss+penalty+learner_config['ownership_weight']*ownership_loss,metrics
