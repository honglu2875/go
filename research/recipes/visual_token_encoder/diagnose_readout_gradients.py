...
def main():
    ...
    p.add_argument('--prefixes',type=int,nargs='+',default=[16])
    ...
    if any(n<1 or n>256 for n in a.prefixes):raise ValueError('Diagnostic history must be in1..256')
    endpoint=max(a.prefixes);bucket=128 if endpoint<=64 else 256
    for shard,game in data.indices['expert',0]:
        off=data.shards[shard]['expert_offsets'];length=int(off[game+1]-off[game])
        if max(32,endpoint)<=length<=bucket:break
    else:raise ValueError('No eligible training episode')
    raw=data.batch([('expert',shard,game)],positions=bucket)
    target=jnp.asarray(raw['policies'][0,endpoint-1]);legal=jnp.asarray(raw['legal'][0,endpoint-1])
    ...
    cases=[]
    for moves in a.prefixes:
        begin=endpoint-moves
        spatial=jnp.asarray(raw['spatial'][:,begin:endpoint]);glob=jnp.asarray(raw['global_features'][:,begin:endpoint])
        actions=jnp.asarray(raw['actions'][:,begin:endpoint]);counts=jnp.asarray([moves],jnp.int32)
        for pooling in ['learned','board_mean']:
            ...
            row={'prefix_moves':moves,'pooling':pooling,'cross_entropy':float(loss),'gradient_l2_by_frame':norm.tolist(),
    ...
    publish(a.output,{'kind':'initial_policy_gradient_frame_allocation','status':'passed','model_snapshot':source.name,
        ...
        'original_game_length':length,'history_lengths':a.prefixes,'common_endpoint':endpoint,'seed':c['seed'],'execution_model':net,'cases':cases,'seconds':time.time()-start,
        'scope':'Full-sized untrained model, same parameters and one fixed training episode, float32 CPU execution. Context windows end at the same current board/target. Differentiate its policy CE with respect to each observed spatial frame. This probes a gradient path, not learnability, playing strength or a training result.'})
