...
def validate(c):
    ...
    if set(c)!=fields or c['architecture']!='causal_visual_policy' or c['encoder'] not in ('overlap_linear','overlap_conv64'):
...
def initialize(seed,c):
    ...
    if c['encoder']=='overlap_conv64':
        for i,inputs in enumerate((22,64)):
            # weight() divides by sqrt(shape[0]); total fan-in is 9*inputs.
            weight(f'encoder.stem{i}.weight',(3,3,inputs,64),scale=1/math.sqrt(3*inputs))
            p[f'encoder.stem{i}.scale']=jnp.ones(64,jnp.float32)
    return p
