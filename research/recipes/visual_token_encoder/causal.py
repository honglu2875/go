"""Pure JAX causal observation/action transformer with one policy output.
...
Action embeddings are tied; an optional shared pointwise logit correction
reuses the current board encoder features without an auxiliary objective.
"""
...
def validate(c):
    ...
    if 'policy_spatial_bias' in c:fields.add('policy_spatial_bias')
    if 'encoder_rematerialize' in c:fields.add('encoder_rematerialize')
    ...
    if ('policy_spatial_bias' in c and (type(c['policy_spatial_bias']) is not bool
            or c['policy_spatial_bias'] and c['encoder']=='overlap_linear')):
        raise ValueError('Spatial policy bias requires a convolutional encoder')
    if ('encoder_rematerialize' in c and (type(c['encoder_rematerialize']) is not bool
            or c['encoder_rematerialize'] and c['encoder']=='overlap_linear')):
        raise ValueError('Encoder rematerialization requires a convolutional encoder')
...
def initialize(seed,c):
    ...
    if c.get('policy_spatial_bias',False):
        # The single policy's local logit correction initially vanishes.
        # Existing parameters and initial logits remain exactly unchanged.
        p['head.local.weight']=jnp.zeros((channels,1),jnp.float32)
...
def spatial_features(p,spatial,c):
    ...
    dtype=jnp.bfloat16 if c['dtype']=='bfloat16' else jnp.float32
    def stem(x,weight,scale):
        x=jax.lax.conv_general_dilated(x.astype(dtype),weight.astype(dtype),
            window_strides=(1,1),padding='SAME',dimension_numbers=('NHWC','HWIO','NHWC'),
            preferred_element_type=jnp.float32)
        return jax.nn.silu(norm(x,scale,c))
    apply=jax.checkpoint(stem) if c.get('encoder_rematerialize',False) else stem
    for i in range(encoder_depth(c)):
        # The next convolution already consumes this dtype. Save that input
        # at the remat boundary, avoiding an unnecessary float32 activation
        # tape in bfloat16 training. The final encoder features stay float32.
        if c.get('encoder_rematerialize',False):x=x.astype(dtype)
        x=apply(x,p[f'encoder.stem{i}.weight'],p[f'encoder.stem{i}.scale'])
...
def observation_encoding(p,spatial,globals,c):
    ...
    tokens=(linear(patches,p['encoder.patch.weight'],c)+p['encoder.patch.bias']+pos+p['token_types.weight'][0]
...
def pack(p,spatial,globals,actions,c,*,with_features=False):
    patches,features=observation_encoding(p,spatial,globals,c);b,t,_,d=patches.shape
    ...
    packed=jnp.concatenate([patches,read,action],2).reshape(b,-1,d)
...
def policy(p,x,size,c,features=None):
    ...
    logits=linear(norm(x,p['head.norm.scale'],c),p['head.actions.weight'][action_indices(size,c)].T,{**c,'dtype':'float32'})
    if c.get('policy_spatial_bias',False):
        if features is None or features.shape!=(*x.shape[:-1],size,size,encoder_channels(c)):
            raise ValueError('Policy needs the current board encoder features')
        # Reuse the SAME convolutional result that formed the visual tokens.
        # One shared pointwise map supplies location-specific evidence; the
        # original readout supplies history/global context and the pass logit.
        local=linear(features,p['head.local.weight'],{**c,'dtype':'float32'})[...,0]+p['head.local.bias']
        local=local.reshape(*x.shape[:-1],size*size)
        logits=logits+jnp.concatenate([local,jnp.zeros_like(logits[...,:1])],-1)
    return logits
...
def first_move(p,spatial,globals,c,*,network_version=0):
    ...
    patches,features=observation_encoding(p,spatial[:,None],globals[:,None],c);patches=patches[:,0];features=features[:,0]
    read=readout_tokens(p,patches,c)
