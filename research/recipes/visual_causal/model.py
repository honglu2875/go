...
def forward(p, observations, actions, counts, c, *, with_cache=False, network_version=0, cache_positions=None):
    ...
    x = norm(x, p['norm'], c)
    states = x.reshape(observations.shape[0], observations.shape[1], spec['stride'], c['width'])
    predictions = heads(p, states[:, :, spec['patches']], states[:, :, :spec['patches']], size, c)
    valid = jnp.arange(observations.shape[1])[None, :] < counts[:, None]
    predictions = jax.tree.map(lambda a: jnp.where(valid.reshape((*valid.shape, *((1,) * (a.ndim - 2)))), a, 0.), predictions)
    if not with_cache:
        return predictions
...
def append_embeddings(p, cache, embeddings, token_counts, allowed_phase, c, network_version):
    ...
    x = embeddings
    keys, values = [], []
    for block, old_k, old_v in zip(p['blocks'], cache['keys'], cache['values']):
        q, k, v = project(x, block, positions, c)
...
def score_continuation(p, cache, future_observations, proposed_actions, counts, c, *, network_version=0):
    ...
    x, updated = append_embeddings(p, cache, x, counts * spec['stride'], phase, c, network_version)
...
def losses(p, batch, c, *, axis_name=None):
