...
def update(params, state, batch, config, *, learning_rate, beta1=.9, beta2=.95,
    ...
    (loss, metrics), grads = jax.value_and_grad(model.losses, has_aux=True)(params, batch, config)
    ...
    return params, state, {**metrics, 'loss': loss, 'grad_norm': grad_norm, 'accepted': finite}
