...
def validate(c):
    fields = 'schema_version kind platform expected_processes expected_devices seed candidate model native_receipt rules slots rounds games_per_round refill max_game_moves cache_positions max_block search sampling worker_cpus suffix_replay fixture_pass_after fixture_pass_spread'
    ...
    if (type(c['games_per_round']) is not int or not c['slots'] <= c['games_per_round'] <= 2048
            or c['games_per_round'] % c['slots'] or type(c['refill']) is not bool
            or type(c['fixture_pass_spread']) is not int or c['fixture_pass_spread'] < 0 or c['fixture_pass_spread'] % 2
            or (c['fixture_pass_after'] is None and c['fixture_pass_spread'] != 0)
            or (c['fixture_pass_after'] is not None and c['fixture_pass_after'] + c['fixture_pass_spread'] > c['max_game_moves'] - 2)):
        raise ValueError('Invalid refill or heterogeneous fixture contract')
...
def run(args, c, report):
    ...
    turn = 0; batches = []; counters = {'games': 0, 'terminal_games': 0, 'capped_games': 0, 'moves': 0, 'terminal_positions': 0,
                                      'outer_steps': 0, 'active_slot_steps': 0}
    ...
    with ThreadPoolExecutor(max_workers=len(c['worker_cpus']), initializer=initialize_worker) as pool:
        while turn < stop:
            turn += 1; runner.reset()
            games = [None] * c['slots']; job_for_slot = [None] * c['slots']; next_job = 0
            rows = [{'actions': [], 'policies': [], 'legal': [], 'search_values': [], 'simulations': [], 'neural': [],
                     'terminal_evaluations': [], 'ending': None} for _ in range(c['games_per_round'])]
            seed_keys = [((turn - 1) * c['games_per_round'] + job) for job in range(c['games_per_round'])]
            rngs = [np.random.default_rng(np.random.SeedSequence([c['seed'], host, 1 + key // c['slots'], key % c['slots']])) for key in seed_keys]
            def fill_slots():
                nonlocal next_job
                for slot in range(c['slots']):
                    if games[slot] is None and next_job < c['games_per_round']:
                        games[slot] = native.Game(json.dumps({**c['rules'], **c['search'], 'history': 1}))
                        job_for_slot[slot] = next_job; next_job += 1
                # Retained paths are safe across episodes: a matching complete
                # action prefix has identical inputs under frozen weights/rules.
                # score() rewinds before writing every divergent suffix. No KV
                # clear or device synchronization is needed to recycle a slot.
            fill_slots(); active = list(range(c['slots']))
            while active:
                counters['outer_steps'] += 1; counters['active_slot_steps'] += len(active)
                ...
                while pending:
                    ...
                    for slot, (request, features) in replies:
                        ...
                        job = job_for_slot[slot]; row = rows[job]; ply = len(row['actions'])
                        fixture_after = None if c['fixture_pass_after'] is None else c['fixture_pass_after'] + (seed_keys[job] % 3) * (c['fixture_pass_spread'] // 2)
                        if fixture_after is not None and ply >= fixture_after: action = area
                        elif ply < c['sampling']['moves']:
                            probability = policy.astype(np.float64) ** (1 / c['sampling']['temperature']); probability /= probability.sum()
                            action = int(rngs[job].choice(area + 1, p=probability))
                        ...
                        row['actions'].append(action); row['policies'].append(policy); row['legal'].append(legal)
                        ...
                        _, _, ended, white_score, _ = game.state()
                        if ended or len(row['actions']) >= c['max_game_moves']:
                            row['ending'] = (bool(ended), float(white_score)); games[slot] = None
                    timings['native_search_seconds'] += time.perf_counter() - started
                active = [slot for slot in active if games[slot] is not None]
                if c['refill'] or not active: fill_slots()
                active = [slot for slot, game in enumerate(games) if game is not None]
            offsets = [0]; values = []; endings = []; summaries = []
            if next_job != c['games_per_round'] or any(row['ending'] is None for row in rows): raise ValueError('Scheduled game queue did not drain')
            for job, row in enumerate(rows):
                terminal, score = row['ending']; length = len(row['actions']); offsets.append(offsets[-1] + length)
                ...
                game_id = f'{SOURCE.name}:host{host}:round{1 + seed_keys[job] // c["slots"]}:slot{seed_keys[job] % c["slots"]}'
            ...
            state = {'schema_version': 1, 'kind': 'visual_selfplay_rank_state', 'snapshot_id': SOURCE.name,
                ...
                'rng_contract': 'Independent PCG64 via SeedSequence[seed,host,1+episode//slots,episode%slots]. Episode IDs are assigned in fixed queue order, independently of physical cache slot. Checkpoint only after the complete registered group drains.'}
    ...
    report.update(status='passed', host_rank=host, jax_rank=rank, host_jax_mapping=mapping, turn=turn,
        ...
        refill=c['refill'], active_slot_fraction=counters['active_slot_steps'] / (counters['outer_steps'] * c['slots']))
