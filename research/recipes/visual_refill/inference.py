...
class Runner:
    def __init__(self, params, c, native, rules, *, slots, cache_positions,
        ...
        self.initializer, self.root_inputs = init, inputs[1:]
        self.root_hash = observation_sha256(root[0])
    ...
    def reset(self):
        """Start a fresh independent episode batch through the same executable."""
        predictions, self.cache = self.initializer(self.params, *self.root_inputs)
        predictions = jax.device_get(predictions)
        self.paths = [() for _ in range(self.slots)]
        self.predictions = [[{k: v[row, 0].copy() for k, v in predictions.items()}] for row in range(self.slots)]
        self.hashes = [[self.root_hash] for _ in range(self.slots)]
