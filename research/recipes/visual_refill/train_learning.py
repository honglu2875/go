...
from gozero.sequence_symmetry import augment
...
def run(args, c, report):
    ...
    if args.resume is None and 'fork' in c:
        import fork
        arrays, saved, fork_lineage = fork.load(SOURCE.parents[2], SOURCE, c, mapping, host, rank, data.manifest['rules'])
        def restore_fork(kind):
            return jax.device_put(definition.unflatten([arrays[f'{kind}_{i:04d}'] for i in range(len(leaves))]), replicated)
        params = restore_fork('p'); state = {'first': restore_fork('m'), 'second': restore_fork('v'),
            'step': jax.device_put(np.asarray(saved['turn'], np.int32), replicated)}
        initial_turn = turn = saved['turn']; counters = dict(saved['counters'])
        ...
        report['initial_parameter_elements_sha256'] = digest_arrays({k: v for k, v in arrays.items() if k.startswith('p_')})
        del arrays
    ...
    with (args.output / 'metrics.jsonl').open('x') as log:
        while turn < stop:
            ...
            record = {'turn': turn, 'bucket': bucket, 'local_entries_sha256': hashlib.sha256(canonical_json(entries)).hexdigest(),
                      'local_symmetries': symmetries.tolist(), **observed}
