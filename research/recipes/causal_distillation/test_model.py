...
class CausalTests(unittest.TestCase):
    ...
    def test_behavior_targets_cannot_train_the_play_trunk(self):
        ...
        batch={'tokens':self.tokens,'lengths':self.lengths,'policies':policies,'observed':moves,'values':outcomes,
               'expert_mask':valid,'behavior_mask':valid,'value_mask':valid,'legal':jnp.ones((*shape,10),jnp.bool_)}
        def component(p,name):return model.losses(p,batch,self.c)[1][name]
