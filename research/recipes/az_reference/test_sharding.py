"""Compare the complete CNN update with and without data-parallel partitioning."""
import unittest
import jax
...
class ShardingTests(unittest.TestCase):
    def test_global_cnn_loss_and_momentum_update_match_one_device(self):
        config={'width':16,'blocks':2,'groups':4,'value_hidden':16,'dtype':'float32'}
        learner={'learning_rate':0.02,'momentum':0.9,'l2':0.0001,'max_grad_norm':5.0,'augment_symmetries':True}
        ...
        bx,bpi,bz=(jax.device_put(a,batch) for a in (x,pi,z))
        single=jax.jit(lambda p,v,x,pi,z,key:model.update(p,v,x,pi,z,key,config,learner))
        distributed=jax.jit(lambda p,v,x,pi,z,key:model.update(p,v,x,pi,z,key,config,learner),
                            in_shardings=(rep,rep,batch,batch,batch,rep),out_shardings=(rep,rep,rep))
        for step in range(3):
            key=jax.random.key(step)
            dk=jax.random.wrap_key_data(jax.device_put(np.asarray(jax.random.key_data(key)),rep))
            p,v,metrics=single(p,v,x,pi,z,key)
            dp,dv,dmetrics=distributed(dp,dv,bx,bpi,bz,dk)
