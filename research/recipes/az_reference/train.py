...
def sgf(game):
    size = game['size']
    text = '(;GM[1]FF[4]CA[UTF-8]SZ[%d]KM[%s]RU[Tromp-Taylor]AP[gozero:az_reference]' % (size, game['komi'])
    if game['white_score'] is not None:
        value = game['white_score']
        result = '0' if value == 0 else ('W' if value > 0 else 'B') + '+' + str(abs(value))
        text += 'RE[' + result + ']'
    if game['truncated']:
        text += 'C[move-limit truncation; excluded from training]'
...
def train(args, c, report):
    ...
    if args.resume:
        ...
        expected = {'key', 'replay_x', 'replay_pi', 'replay_z', 'replay_meta'} | {f'{kind}_{i:04d}' for kind in ('p','v') for i in range(leaves_count)}
        ...
        for target, name in ((replay_x,'replay_x'), (replay_pi,'replay_pi'), (replay_z,'replay_z'), (replay_meta,'replay_meta')):
    ...
    def inference(p, features, active):
        logits, values = model.apply(p, features, net)
        return logits, values, jnp.sum(active, dtype=jnp.int32)

    def update(p, v, x, pi, z, rng):
        next_key, draw = jax.random.split(rng)
        p, v, metrics = model.update(p, v, x, pi, z, draw, net, learner)
        return p, v, next_key, metrics

    before = time.perf_counter()
    forward = jax.jit(inference, in_shardings=(replicated, batched, batched), out_shardings=(batched, batched, replicated)).lower(
        params, global_batch(np.zeros((actor['games'], size, size, channels), np.float32)),
        global_batch(np.zeros(actor['games'], np.int32))).compile()
    report['forward_compile_seconds'] = time.perf_counter()-before
    before = time.perf_counter()
    step = jax.jit(update, in_shardings=(replicated, replicated, batched, batched, batched, replicated),
        ...
        global_batch(np.zeros(learner['batch_size'], np.float32)), key).compile()
    ...
    def save(engine, current_turn):
        ...
        arrays = {**{f'p_{i:04d}': value for i, value in enumerate(p)}, **{f'v_{i:04d}': value for i, value in enumerate(v)},
                  ...
                  'replay_z': replay_z[:count], 'replay_meta': replay_meta[:count]}
        ...
        manifest_sha = checkpoints.write(path, state=state, arrays=arrays, actors=engine.checkpoint())
    ...
    with Actors(actor, args.native_receipt.parent/receipt['filename'], receipt['binary_sha256'], checkpoint=actor_state) as engine:
        with (output/'metrics.jsonl').open('x') as log:
            while turn < stop_turn:
                ...
                while True:
                    before = time.perf_counter()
                    logits, values, active = forward(params, global_batch(batch.features), global_batch(batch.active.astype(np.int32)))
                    active = int(replica(active))
                    ...
                    if active == 0:
                        ...
                        break
                    logits, values = local_batch(logits), local_batch(values)
                    counters['inference_seconds'] += time.perf_counter()-before
                ...
                if ready:
                    for _ in range(learner['updates_per_turn']):
                        ...
                        before = time.perf_counter()
                        params, velocity, key, metrics = step(params, velocity, global_batch(replay_x[indices]), global_batch(replay_pi[indices]), global_batch(replay_z[indices]), key)
...
def main():
    ...
    report = {'schema_version': 1, 'kind': 'az_reference_training', 'status': 'running', 'snapshot_id': ROOT.name,
