...
def config(**kw):
    return dict(architecture='causal_visual_policy',width=16,layers=2,mlp_hidden=24,
        ...
        encoder_attention_blocks=1,encoder_attention_heads=2,encoder_attention_mlp_hidden=24,encoder_rope_theta=10000.,**kw)
...
class SharedBoardTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.p['head.context.q.weight']=jnp.linspace(-.15,.15,cls.c['width']*4).reshape(cls.c['width'],4)
    ...
    def test_zero_correction_preserves_control(self):
        c=self.c;off={**c,'policy_spatial_bias':False,'policy_context_dim':0}
        candidate=causal.initialize(17,c);control=causal.initialize(17,off)
        self.assertEqual(set(candidate)-set(control),{'head.local.weight','head.local.bias','head.context.q.weight','head.context.k.weight'})
        ...
        self.assertEqual(added,c['encoder_width']+1+c['policy_context_dim']*(c['width']+c['encoder_width']))
    ...
    def test_spatial_path_learns_before_encoder_changes(self):
        ...
        off={**c,'policy_spatial_bias':False,'policy_context_dim':0};control=causal.initialize(17,off)
    ...
    def test_readout_spatial_permutation_and_pass(self):
        ...
        base=causal.policy(p,h,3,{**c,'policy_spatial_bias':False,'policy_context_dim':0})
    ...
    def test_context_zero_init_preserves_spatial_parent_and_learns(self):
        c=self.c;off={**c,'policy_context_dim':0}
        p=causal.initialize(17,c);parent=causal.initialize(17,off)
        for key in parent:np.testing.assert_array_equal(p[key],parent[key])
        np.testing.assert_array_equal(causal.forward(p,self.s,self.g,self.a,self.n,c),
                                      causal.forward(parent,self.s,self.g,self.a,self.n,off))
        def loss(q):return -jax.nn.log_softmax(causal.forward(q,self.s,self.g,self.a,self.n,c),axis=-1)[0,1,2]
        gradient=jax.jit(jax.grad(loss))(p)
        self.assertGreater(float(jnp.linalg.norm(gradient['head.context.q.weight'])),1e-6)
        np.testing.assert_array_equal(gradient['head.context.k.weight'],0)
        learned={k:v-.01*gradient[k] for k,v in p.items()}
        second=jax.jit(jax.grad(loss))(learned)
        self.assertGreater(float(jnp.linalg.norm(second['head.context.k.weight'])),1e-6)

    def test_context_correction_depends_on_temporal_state(self):
        p=self.p;c=self.c;f=encoder.spatial({k[8:]:v for k,v in p.items() if k.startswith('encoder.')},self.s,c)[:,0]
        h=jnp.ones((2,c['width']));other=h.at[:,::2].set(-1)
        off={**c,'policy_context_dim':0}
        correction=lambda x:causal.policy(p,x,3,c,f)-causal.policy(p,x,3,off,f)
        self.assertGreater(float(jnp.max(jnp.abs(correction(h)-correction(other)))),1e-6)
        np.testing.assert_array_equal(correction(h)[:,-1],0)

    def test_attention_reaches_distant_points_but_not_other_boards(self):
        c=self.c;params=encoder.initialize(3,c)
        a={k[len('attention.'):]:v[0] for k,v in params.items() if k.startswith('attention.')}
        x=jax.random.normal(jax.random.key(28),(2,3,3,c['encoder_width']))
        gradient=jax.grad(lambda x:jnp.mean(encoder.attention_block(x,a,c)[0,0,0]**2))(x)
        self.assertGreater(float(jnp.linalg.norm(gradient[0,2,2])),1e-8)
        np.testing.assert_array_equal(gradient[1],0)

    def test_shared_attention_gradient_sums_over_passes(self):
        c=self.c;params=encoder.initialize(3,c)
        a={k[len('attention.'):]:v[0] for k,v in params.items() if k.startswith('attention.')}
        x=jax.random.normal(jax.random.key(29),(1,3,3,c['encoder_width']))
        def loss(pair):return jnp.mean(encoder.attention_block(encoder.attention_block(x,pair[0],c),pair[1],c)**2)
        separate=jax.grad(loss)((a,a));shared=jax.grad(lambda p:loss((p,p)))(a)
        for key in a:np.testing.assert_allclose(shared[key],separate[0][key]+separate[1][key],atol=2e-6,rtol=2e-4)
        self.assertGreater(float(jnp.linalg.norm(separate[0]['q.weight'])),1e-8)
        self.assertGreater(float(jnp.linalg.norm(separate[1]['q.weight'])),1e-8)

    def test_2d_rope_preserves_relative_scores_under_translation(self):
        c=self.c;r=jnp.repeat(jnp.arange(3,dtype=jnp.float32),3);col=jnp.tile(jnp.arange(3,dtype=jnp.float32),3)
        q=jax.random.normal(jax.random.key(5),(1,9,2,8));k=jax.random.normal(jax.random.key(6),(1,9,2,8))
        def score(row,col):return jnp.einsum('bihd,bjhd->bhij',encoder.rope_2d(q,row,col,c),encoder.rope_2d(k,row,col,c))
        np.testing.assert_allclose(score(r,col),score(r+7,col+13),atol=2e-5,rtol=2e-5)
