"""Combine two audited encoder-attention pairs without selecting the better seed."""
import argparse
import json
...
def main():
    ...
    for path,h in a.pair:
        ...
        if contrast['status']!='passed' or contrast['kind']!='encoder_attention_contrast':raise ValueError('Invalid spatial audit')
    ...
    out={'status':'passed','kind':'two_seed_encoder_attention_replication','operator_snapshot':SOURCE.name,
    ...
    axes[0].set_ylabel('Validation KL');axes[1].set_ylabel('Attention minus control KL')
    axes[1].axhline(0,color='black',linewidth=.8)
    for ax in axes:ax.set_xlabel('Optimizer updates');ax.grid(alpha=.2);ax.legend(fontsize=9)
    fig.suptitle('Within-board encoder attention: both paired seeds')
    fig.savefig(a.output/'learning.png',dpi=160)
    fig.savefig(a.output/'learning.svg',metadata={'Date':None})
    plt.close(fig)
    lines=['The encoder-attention replication retains both independent seeds. Configuration and numerical source change only by seed between corresponding runs; actual game and D4 draws match within each pair.',
           '', '| Seed | Position exposures per arm | Control KL | Attention KL | Relative KL improvement | Decode latency ratio |',
           '|---|---:|---:|---:|---:|---:|']
    for pair in pairs:
    ...
    lines+=['',f"Mean KL: control {control_mean:.6f}; attention {candidate_mean:.6f}. Relative improvement of the mean: {out['relative_improvement_of_mean_kl']:.2%}.",
