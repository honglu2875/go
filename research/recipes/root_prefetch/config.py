...
def validate(c):
    fields(c, 'schema_version platform expected_processes expected_devices seed selfplay_turns checkpoint_every log_every actors model learner root_prefetch', 'config')
    prefetch = c['root_prefetch']
    fields(prefetch, 'mode limit', 'root_prefetch')
    if prefetch['mode'] not in ('off', 'queued', 'wide') or not integer(prefetch['limit'], 1, 64):
        raise ValueError('Invalid root prefetch execution settings')
    if prefetch['mode'] != 'off' and c['actors']['gumbel'] is None:
        raise ValueError('Root prefetch requires Gumbel search')
    ...
    fields(n, 'width blocks groups value_hidden dtype score_utility_factor score_scale', 'model')
    if not real(n['score_utility_factor'],0,2) or not real(n['score_scale'],0.001,100):
        raise ValueError('Invalid score utility configuration')
    ...
    fields(l, 'batch_size replay_capacity warmup_rows learning_rate momentum l2 max_grad_norm augment_symmetries updates_per_turn ownership_weight score_weight', 'learner')
    if not real(l['score_weight'],0,10): raise ValueError('Invalid score supervision coefficient')
    ...
    fields(a, 'size komi scoring history games workers worker_cpus simulations cpuct fpu_reduction score_utility gumbel max_search_edges max_game_moves dirichlet_alpha dirichlet_fraction temperature_early temperature_late temperature_moves seed actor_offset', 'actors')
    if a['gumbel'] is not None:
        g=a['gumbel']
        fields(g,'max_considered_actions value_scale maxvisit_init rescale_values gumbel_scale','Gumbel')
        if (not integer(g['max_considered_actions'],1,4096) or not real(g['value_scale'],0.0001,100)
                or not real(g['maxvisit_init'],0,1_000_000) or not real(g['gumbel_scale'],0,10)
                or type(g['rescale_values']) is not bool or not integer(a['simulations'],1,1_000_000)
                or a['cpuct']!=0 or a['fpu_reduction'] is not None or a['dirichlet_fraction']!=0
                or a['temperature_early']!=0 or a['temperature_late']!=0 or a['temperature_moves']!=0):
            raise ValueError('Invalid Gumbel search or unused PUCT/temperature settings')
    if a['score_utility']!={'factor':n['score_utility_factor'],'scale':n['score_scale']}:
        raise ValueError('Neural and terminal search utility must match')
    ...
    if 4 * feature_width * a['games'] * prefetch['limit'] > 2**30:
        raise ValueError('Root prefetch features exceed one GiB')
