...
def main():
    ...
    if read_json(args.config).get('kind') == 'visual_deferred_loop_profile':
        from profile_loop import entry
        return entry(args)
    ...
    if read_json(args.config).get('kind') == 'visual_inference_service':
        ...
        return entry(args)
    if read_json(args.config).get('kind') == 'visual_speculation_profile':
        from profile_speculate import entry
        return entry(args)
