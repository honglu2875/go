...
def losses(p,b,c,*,axis_name=None):
    # The ordinary inference path has no helper or batch-statistics operation.
    # Keep the legacy parameter tree to preserve exact initialization and source
    # outside this objective; unused helper slots are reported separately.
    main=logits(p,b,c,training=False,axis_name=axis_name)
    m=averages(total_metrics(main,b,axis_name))
