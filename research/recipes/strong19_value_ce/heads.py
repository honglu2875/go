...
def signed_target_cross_entropy(logits,b,*,axis_name=None):
    """Stable signed-target surrogate using the existing three-logit head.

    Only the teacher's signed expectation y is stored, not its WDL vector.
    [(1+y)/2,(1-y)/2,0] preserves that expectation and removes the unused
    third-outcome degree of freedom. This is not a reconstruction of teacher
    WDL probabilities. Mask padded targets/logits before any arithmetic.
    """
    if logits.shape!=(*b['values'].shape,3):raise ValueError('Value logit/target shapes differ')
    live=jnp.arange(logits.shape[1])[None,:]<b['counts'][:,None]
    y=jnp.where(live,b['values'],0.).astype(jnp.float32)
    z=jnp.where(live[...,None],logits,0.).astype(jnp.float32)
    q=jnp.stack(((1+y)/2,(1-y)/2,jnp.zeros_like(y)),axis=-1)
    logp=jax.nn.log_softmax(z,axis=-1);p=jnp.exp(logp)
    totals=dict(count=jnp.sum(live),ce=jnp.sum(jnp.where(live,-jnp.sum(q*logp,axis=-1),0.)),
        probability_win=jnp.sum(jnp.where(live,p[...,0],0.)),
        probability_loss=jnp.sum(jnp.where(live,p[...,1],0.)),
        probability_neutral=jnp.sum(jnp.where(live,p[...,2],0.)),
        neutral_over_99=jnp.sum(live&(p[...,2]>.99)))
    if axis_name is not None:totals=jax.tree.map(lambda x:jax.lax.psum(x,axis_name),totals)
    denominator=jnp.maximum(totals['count'],1)
    return {k:v/denominator for k,v in totals.items() if k!='count'}
