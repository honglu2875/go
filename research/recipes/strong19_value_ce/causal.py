...
def validate(c):
    ...
    optional={'policy_context_dim','encoder_attention_blocks','encoder_attention_heads','encoder_attention_mlp_hidden','encoder_rope_theta','first_pass_aux_weight','policy_readout_kind','policy_refinement_dim'}
    ...
    if c.get('policy_readout_kind','bilinear') not in ('bilinear','additive'):raise ValueError('Unknown spatial readout')
    if c.get('policy_readout_kind','bilinear')=='additive' and not c.get('policy_context_dim',0):raise ValueError('Additive readout requires a contextual projection')
    if type(c.get('policy_refinement_dim',0)) is not int or not 0<=c.get('policy_refinement_dim',0)<=256:raise ValueError('Invalid policy refinement width')
...
def initialize(seed,c):
    ...
    if c.get('policy_context_dim',0):
        ...
        if c.get('policy_readout_kind','bilinear')=='additive':
            p['head.context.score.weight']=jnp.zeros((dim,1),jnp.float32)
    if c.get('policy_refinement_dim',0):
        import spatial_readout
        p.update(spatial_readout.initialize(seed,c))
    return p
...
def observation_encoding(p,spatial,globals,c):
    ...
    return token[:,:,None,:],features if c['policy_spatial_bias'] or c.get('policy_context_dim',0) or c.get('policy_refinement_dim',0) else None
...
def policy(p,x,size,c,features=None):
    ...
    if c.get('policy_refinement_dim',0):
        import spatial_readout
        h=spatial_readout.refine(p,h,features,c)
    ...
    if c.get('policy_context_dim',0):
        ...
        if c.get('policy_readout_kind','bilinear')=='additive':
            hidden=jax.nn.silu(k+q[...,None,None,:])
            correction=linear(hidden,p['head.context.score.weight'],{**c,'dtype':'float32'})[...,0]
        else:
...
def forward(p,spatial,globals,actions,counts,c,*,with_cache=False,cache_positions=None,network_version=0,with_hidden=False):
    ...
    output={'policy':logits,'latent':jnp.where(live[:,:,None],norm(x,p['head.norm.scale'],c),0.)} if with_hidden else logits
    if not with_cache:return output
    ...
    return output,{'keys':store(cache_values[0]),'values':store(cache_values[1]),'lengths':lengths,'valid':valid,
                   'network_version':jnp.asarray(network_version,jnp.uint32)}


def first_move(p,spatial,globals,c,*,network_version=0,with_hidden=False):
    ...
    logits=policy(p,x[:,-1],spatial.shape[1],c,features)
    output={'policy':logits,'latent':norm(x[:,-1],p['head.norm.scale'],c)} if with_hidden else logits
    return output,{
...
def append_move(p,cache,previous_action,spatial,globals,c,*,attention_positions,active=None,network_version=0,with_hidden=False):
...
"""
    ...
    logits=jnp.where(write[:,None],logits,0.)
    output={'policy':logits,'latent':jnp.where(write[:,None],norm(x[:,-1],p['head.norm.scale'],c),0.)} if with_hidden else logits
