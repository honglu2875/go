...
def validate(c):
    ...
    fields(l, 'batch_size replay_capacity warmup_rows learning_rate momentum l2 max_grad_norm augment_symmetries updates_per_turn', 'learner')
    ...
    fields(a, 'size komi history games workers worker_cpus simulations cpuct max_search_edges max_game_moves dirichlet_alpha dirichlet_fraction temperature_early temperature_late temperature_moves seed actor_offset', 'actors')
    ...
    estimate=4*(feature_width+a['size']**2+1)*(l['replay_capacity']+a['games']*a['max_game_moves'])
