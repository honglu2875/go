...
def validate(c):
    fields='schema_version platform seed games horizon samples controller_cpus actors model candidate expected_processes expected_devices qualification_moves_per_game timed_moves_per_game orders warmup_calls profile profile_moves_per_game'
    ...
    if c['games']*c['timed_moves_per_game']*3*len(c['orders'])>2**20:
...
def play(native,execute,params,key,model_id,c,mode,target,output,label,synchronize=None):
    import jax
    import numpy as np
    ...
    horizon,samples,_,_=MODES[mode];remaining=np.full(c['games'],target,np.int64)
    records=[];counts=Counter();before=time.perf_counter();cpu_before=time.process_time()
    for packet in range(target):
        ...
        mark=time.perf_counter();arrays=execute(mode,params,tokens,lengths,episodes,key,stones,legal)
        ...
        counts['prefill_token_slots']+=tokens.size
    ...
    report={'mode':mode,'label':label,'target_moves_per_game':target,'elapsed_seconds':elapsed,
        'process_cpu_seconds':cpu,'average_active_cpu_cores':cpu/elapsed,'setup_seconds':setup,
...
def run(args,c,report):
    ...
    without_temperatures=lambda d:{k:v for k,v in d.items()if not k.endswith('_temperature')}
    if without_temperatures(c['model'])!=without_temperatures(trained['config']['model']):
    ...
    executables={};report['compilation']={}
    for name,(horizon,samples,coupling,oracle)in MODES.items():
        ...
        for _ in range(c['warmup_calls']):jax.device_get(compiled(*dummy))
    ...
    def execute(mode,p,t,l,e,k,s,legal):
        return jax.device_get(executables[mode](p,jax.device_put(t,batched),jax.device_put(l,batched),
            jax.device_put(e,batched),k,jax.device_put(s,batched),jax.device_put(legal,batched)))
    ...
    def run_mode(mode,target,label,stream):
        ...
        return play(native,execute,params,draw,model_id,c,mode,target,args.output,label,synchronize=sync)
            ...
            try:
                ...
                try:
                    profiled,events,_=play(native,execute,params,jax.device_put(jax.random.fold_in(key,1),rep),
                        model_id,c,'behavior',c['profile_moves_per_game'],args.output,'profile-behavior')
                finally:
...
def main():
    ...
    report={'schema_version':1,'kind':'equal_prefix_trace_throughput','status':'running','snapshot_id':ROOT.name,
