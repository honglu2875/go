"""Combine two audited spatial-readout pairs without selecting the better seed."""
import argparse
import json
...
def main():
    ...
    for path,h in a.pair:
        ...
        if contrast['status']!='passed' or contrast['kind']!='minimal_spatial_readout_contrast':raise ValueError('Invalid spatial audit')
    ...
    out={'status':'passed','kind':'two_seed_spatial_readout_replication','operator_snapshot':SOURCE.name,
    ...
    axes[0].set_ylabel('Validation KL');axes[1].set_ylabel('Spatial minus control KL')
    axes[1].axhline(0,color='black',linewidth=.8)
    for ax in axes:ax.set_xlabel('Optimizer updates');ax.grid(alpha=.2);ax.legend(fontsize=9)
    fig.suptitle('Spatial policy readout: both paired seeds')
    fig.savefig(a.output/'learning.png',dpi=160)
    fig.savefig(a.output/'learning.svg',metadata={'Date':None})
    plt.close(fig)
    lines=['The spatial-readout replication retains both independent seeds. Configuration and numerical source change only by seed between corresponding runs; actual game and D4 draws match within each pair.',
