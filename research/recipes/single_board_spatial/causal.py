...
def validate(c):
    ...
    if set(c)!=required|{'policy_spatial_bias'} or c['architecture']!='causal_visual_policy':raise ValueError('Unknown shared-board model contract')
...
def observation_encoding(p,spatial,globals,c):
    ...
    return token[:,:,None,:],features if c['policy_spatial_bias'] else None
