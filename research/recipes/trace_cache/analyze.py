...
MODES=('sequential','behavior','joint','cached_sequential','cached_behavior','cached_joint')
...
def audit_segment(directory,row,c,model_id):
    ...
    mode=row['mode'];require(mode in MODES,'Unknown mode');base=mode.removeprefix('cached_');cached=mode.startswith('cached_')
    ...
    carry_bytes=games*(2*samples*c['model']['blocks']*2*c['model']['max_tokens']*c['model']['width']*4+2*actions*4)if cached else 0
...
def audit_host(root,directory,protocol,host):
    ...
    require(result['kind']=='retained_kv_trace_throughput' and result['status']=='passed' and 'error'not in result
    ...
    require(result['candidate']==descriptor and result['candidate_sha256']==sha256(source/c['candidate'])
            and trained['model_code_sha256']==sha256(source/'research/recipes/trace_cache/model.py')
...
def main():
    ...
    report={'schema_version':1,'kind':'retained_kv_trace_audit','status':'passed','analysis_snapshot':SOURCE.name,
