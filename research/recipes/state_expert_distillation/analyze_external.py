#!/usr/bin/env python3
"""Audit registered expert-architecture training and fresh paired real-KataGo games."""
import argparse
import json
...
def paired_comparison(control, candidate, openings, c):
    ...
    arms = {}; completed = {}
    for name, games in (('history', control), ('state', candidate)):
    ...
    difference = np.stack((arms['state'][:, 0] - arms['history'][:, 1], arms['state'][:, 1] - arms['history'][:, 0]), axis=-1)
    ...
    positive = interval[0] > c['minimum_state_minus_history_lower_endpoint_strictly_above']
    return {'opening_units': n, 'completed_games': completed,
            ...
            'state_minus_history_score_bounds': difference.mean(axis=0).tolist(), 'paired_bootstrap_95_outer_interval': interval,
            ...
            'per_opening': [{'pair': i, 'opening': opening, 'history_score_bounds': arms['history'][i].tolist(),
                             'state_score_bounds': arms['state'][i].tolist(), 'difference_bounds': difference[i].tolist()}
...
def audit_suite(root, protocol, mode, directory, training, protocol_sha):
    ...
    require(suite['kind'] == 'state_expert_katago_suite' and suite['status'] == 'passed' and 'error' not in suite
            ...
            and descriptor['training_snapshot'] == arm['snapshot_id'] and trained['config']['model']['board_mode'] == 'exact' and trained['config']['model']['architecture'] == mode and trained['config']['model']['value_objective'] == 'mse',
    ...
    attempts = [p for p in (root / 'runs/eval').glob('*/result.json')
                if read_json(p).get('kind') == 'state_expert_katago_suite' and read_json(p).get('arm') == mode
    ...
    for declaration, recorded in zip(arm['evaluation_panels'], suite['panels']):
        ...
        for child_decl in spec['matches']:
            ...
            require(child['candidate_adapter'] == 'eval/state_expert_gtp.py' and child['candidate_scoring_profile'] == 'pass_alive_area'
...
def main():
    ...
    p.add_argument('--history-suite', required=True); p.add_argument('--state-suite', required=True)
    ...
    require(protocol['kind'] == 'state_expert_distillation_pilot' and sha256(a.training_audit) == a.expected_training_audit_sha256,
    ...
    report = {'schema_version': 1, 'kind': 'state_expert_distillation_pilot_analysis', 'status': 'failed',
    ...
    try:
        ...
        for mode, name in (('history', a.history_suite), ('state', a.state_suite)):
        ...
        book_path = SOURCE / 'eval/state_expert_distillation/openings.json'
        ...
        primary = paired_comparison(groups['history'], groups['state'], book['early'], protocol['external_comparison'])
