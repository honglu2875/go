"""Native/transcript replay helpers with explicit state-expert loaded identities.
...
def loaded_identity(folder, game, source, spec, descriptor, trained, native):
    ...
    require(game['candidate_version'] == 'gozero-state-expert-native-leaf-v1'
            and ready['adapter'] == 'state_expert_native_leaf' and ready['expert_architecture'] == trained['config']['model']['architecture']
            and ready['fixed_observer'] == trained['training_result']['fixed_observer'] and ready['observer_used_as_search_policy'] is False
            and ready['state_feature_channels'] == 9
    ...
    require(set(ready['helper_code_sha256']) == {'history_model.py', 'spatial_model.py', 'state_features.py'}
            and all(sha256(trained['snapshot'] / trained['manifest']['recipe'] / name) == digest
                    for name, digest in ready['helper_code_sha256'].items()), 'Loaded model helpers differ')
