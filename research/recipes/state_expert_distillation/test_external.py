"""The external criterion requires both arms to complete and preserves caps."""
import unittest
from analyze_external import paired_comparison


class PairedExternalTest(unittest.TestCase):
    openings = [[str(i), str(i + 1)] for i in range(32)]
    criterion = {'opening_units': 32, 'bootstrap_seed': 91310057, 'bootstrap_replicates': 20000,
                 'confidence': .95, 'minimum_each_arm_completion_fraction': .95,
                 'minimum_state_minus_history_lower_endpoint_strictly_above': 0., 'scope': 'Synthetic fixture'}
    ...
    def test_caps_in_either_arm_fail_completion_when_over_five_percent(self):
        for arm in (0, 1):
            games = [self.games(0.), self.games(1.)]
            for game in games[arm][:4]:
                game.update(status='truncated', candidate_points=None)
            ...
            self.assertGreater(result['state_minus_history_score_bounds'][1], result['state_minus_history_score_bounds'][0])
    ...
    def test_assigned_caps_and_missing_colors_are_rejected(self):
        ...
        with self.assertRaises(ValueError):
            paired_comparison(control, candidate, self.openings, self.criterion)
