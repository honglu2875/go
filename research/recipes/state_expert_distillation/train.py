#!/usr/bin/env python3
"""Complete expert-only learner with a separately pinned, unchanged observer."""
import argparse
import hashlib
import json
import os
...
from gozero.causal_artifacts import validate as validate_observer
...
def train(args, c, report):
    ...
    import model
    observer_path = SOURCE / c['observer']['descriptor']
    if checkpoints.sha256(observer_path) != c['observer']['sha256']:
        raise ValueError('Frozen observer descriptor changed')
    observer_descriptor = read_json(observer_path)
    observer_identity = validate_observer(SOURCE.parents[2], observer_descriptor)
    report['fixed_observer'] = {'descriptor_sha256': c['observer']['sha256'],
                              'training_snapshot': observer_descriptor['training_snapshot'],
                              'weights_sha256': observer_descriptor['model_export_sha256'],
    ...
    data = Dataset(dataset_path, c['dataset']['manifest_sha256'], rank=rank, world=world, board_mode=c['model']['board_mode'])
    ...
    report.update(jax_rank=rank, world_size=world, jax_version=jax.__version__, devices=[str(d) for d in devices],
                  ...
                  teacher_cost=data.parent_manifest['spec']['teacher_cost'], board_mode=c['model']['board_mode'],
                  value_objective=c['model']['value_objective'], expert_architecture=c['model']['architecture'])
    ...
    initial_digest=hashlib.sha256(canonical_json(schema))
    for array in first:initial_digest.update(np.ascontiguousarray(array).tobytes())
    report['initial_parameter_elements_sha256']=initial_digest.hexdigest()
    ...
    turn = 0
    counters = {'updates': 0, 'expert_token_exposures': 0,
    ...
    dummy = global_batch(data.sample(random, learner['games_per_host']))
    ...
    def heldout(split):
        ...
        batch_size = learner['games_per_host']
        maximum = int(np.max(mh.process_allgather(np.asarray(len(entries), np.int32))))
        sums = {k: 0. for k in ('play_loss', 'value_loss', 'target_entropy', 'play_top1', 'illegal_probability')}
        counts = {k: 0 for k in ('expert_tokens', 'value_tokens')}
        for begin in range(0, maximum, batch_size):
            ...
            for key in sums:
                population = 'value_tokens' if key == 'value_loss' else 'expert_tokens'
    ...
    with (args.output / 'metrics.jsonl').open('x') as log:
        while turn < stop:
            ...
            batch = data.sample(random, learner['games_per_host'])
    ...
    validate_observer(SOURCE.parents[2], observer_descriptor)
...
def main():
    ...
    report = {'schema_version': 1, 'kind': 'state_expert_distillation', 'snapshot_id': SOURCE.name,
              'status': 'running', 'claims_rl_sample_efficiency': False, 'claims_go_strength': False,
              'behavior_training_enabled': False, 'observed_moves_used_as_expert_targets': False}
    distributed = False
    try:
        import jax
        if c['platform'] == 'tpu':
