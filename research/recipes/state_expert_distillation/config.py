...
def validate(c):
    if set(c) != set('schema_version platform expected_processes expected_devices seed dataset observer model learner steps checkpoint_every log_every eval_every'.split()):
    ...
    o = c['observer']
    if (set(o) != {'descriptor', 'sha256'} or not isinstance(o['descriptor'], str)
            or not o['descriptor'].startswith('eval/') or '..' in o['descriptor'].split('/')
            or not isinstance(o['sha256'], str) or len(o['sha256']) != 64
            or any(x not in '0123456789abcdef' for x in o['sha256'])):
        raise ValueError('Frozen observer identity required')
    if set(m) != set('size komi width heads blocks max_tokens dtype expert_temperature behavior_temperature board_width board_blocks board_mode value_objective architecture state_width state_heads state_blocks state_value_hidden state_max_board_size'.split()):
        raise ValueError('Unknown model configuration')
    if m['value_objective'] != 'mse' or m['board_mode'] != 'exact' or m['architecture'] not in ('history', 'state'):
        raise ValueError('This comparison requires MSE, exact boards and a declared expert architecture')
    for key, lo, hi in [('state_width', 8, 512), ('state_heads', 1, 32), ('state_blocks', 1, 16),
                        ('state_value_hidden', 8, 512), ('state_max_board_size', 9, 25)]:
        if type(m[key]) is not int or not lo <= m[key] <= hi:
            raise ValueError('Invalid ' + key)
    if m['state_width'] % m['state_heads']:
        raise ValueError('Spatial head dimension must divide width')
    ...
    if set(l) != set('games_per_host learning_rate warmup_steps beta1 beta2 epsilon weight_decay grad_clip'.split()):
        raise ValueError('Unknown optimizer configuration')
    if type(l['games_per_host']) is not int or not 1 <= l['games_per_host'] <= 128:
    ...
    if l['games_per_host'] * c['expected_processes'] % c['expected_devices']:
