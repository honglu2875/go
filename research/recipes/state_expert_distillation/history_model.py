...
def head_outputs(p,h,observed_counts,board,*,return_value_logits=False):
    ...
    value_logits=(h@p['value']+pooled@p['board_value'])[...,0]
    value=jnp.tanh(value_logits)
    ...
    if return_value_logits:return play,behavior,value,value_logits
...
def predictions(p,batch,c,*,return_value_logits=False):
    ...
    return head_outputs(p,h,context,board,return_value_logits=return_value_logits)


def value_objective(value_logits,target,kind):
    """The BCE factor matches MSE's gradient at a neutral value prediction."""
    if kind=='mse':return (jnp.tanh(value_logits)-target)**2
    if kind=='bce2':
        # P(win) = sigmoid(2*r) = (tanh(r)+1)/2. No atanh/clipping needed.
        return 2*(jax.nn.softplus(2*value_logits)-(target+1)*value_logits)
    raise ValueError('Unknown value objective')


def losses(p,batch,c):
    play,behavior,value,value_logits=predictions(p,batch,c,return_value_logits=True)
    ...
    training_value_loss=jnp.sum(value_mask*value_objective(value_logits,batch['values'],c.get('value_objective','mse')))/jnp.maximum(1.,value_count)
    entropy=-jnp.sum(batch['policies']*jnp.log(jnp.maximum(batch['policies'],1e-30)),axis=-1)
    metrics={'play_loss':expert,'behavior_loss':observed,'value_loss':value_loss,'training_value_loss':training_value_loss,
    ...
    # value_loss remains outcome MSE in both arms, so validation is comparable.
    return expert+observed+training_value_loss,metrics
