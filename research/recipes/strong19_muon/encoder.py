...
def spatial(p,s,c,*,with_first=False):
    ...
    if count:
        ...
        for i in range(c['encoder_passes']):
            x,_=jax.lax.scan(segment,x,(convs,attn))
            if i==0:first=x
    else:
        for i in range(c['encoder_passes']):
            x,_=jax.lax.scan(scan,x,blocks)
            if i==0:first=x
    result=x.reshape(batch,time,h,w,c['encoder_width'])
    return (result,first.reshape(result.shape)) if with_first else result


def connect(p,features,g,c):
    x=linear(features,p['compress.weight'],c)+p['compress.bias']
    x=linear(x.reshape(*features.shape[:2],-1),p['flat.weight'],c)+p['flat.bias']
    x=norm(x,p['token.norm.scale'],p['token.norm.bias'],c)
    return x+linear(g,p['global.weight'],c)+p['type']
