...
def draft_blocks(length,tile,valid_length):
    from draft_mask import DraftMask,visible
    from jax.experimental.pallas.ops.tpu.splash_attention import splash_attention_mask as masks
    from jax.experimental.pallas.ops.tpu.splash_attention import splash_attention_mask_info as mask_info
    positions=np.arange(length);allowed=visible(positions[:,None],positions[None,:],valid_length)
    expected=sum(bool(allowed[q:q+tile,k:k+tile].any()) for q in range(0,length,tile) for k in range(0,length,tile))
    mask=masks.MultiHeadMask([DraftMask((length,length),valid_length)])
    for processor in (mask_info.process_mask,mask_info.process_mask_dkv):
        info,_=processor(mask,(tile,tile),head_shards=1,q_seq_shards=1)
        if np.count_nonzero(info.block_mask)!=expected:raise ValueError('Draft sparse block schedule differs from independent enumeration')
    return expected


def count(graph,*,observation_stride=0,valid_length=None,draft_valid_length=None):
    ...
    def walk(graph,m=1):
        ...
        for eq in graph.eqns:
            ...
            if name=='scan':walk(p['jaxpr'],m*p['length'])
                ...
                if label in ('splash_mha_fwd_residuals','splash_mha_fwd_no_residuals'):phase='fwd';tile=meta['block_q'];other=meta['block_kv'];dots=2
                ...
                blocks=(draft_blocks(length,tile,draft_valid_length) if draft_valid_length else active_attention_blocks(length,tile,observation_stride,valid_length))
...
def main():
    ...
    p.add_argument('--attention-mask-qualification',type=Path)
    ...
    if c['model'].get('observation_attention')=='board':
        if args.attention_mask_qualification is None or read_json(args.attention_mask_qualification)['status']!='passed':
            raise ValueError('Board mask needs its own TPU numerical qualification')
    for label,model in [('cnn',base['model']),('transformer',c['model'])]:
        ...
        for length in c['dataset']['buckets']:
            ...
            for remat in (False,True):
                ...
                stride=causal.layout(9,net)['stride'] if net.get('observation_attention')=='board' else 0
                row=count(graph,observation_stride=stride,valid_length=length*stride if stride else None,
                    draft_valid_length=3*length if net.get('first_pass_aux_weight',0) else None);cases[f'{label}-{length}-remat{int(remat)}']=row
    ...
    result={'kind':'differentiated_training_matrix_arithmetic','operator_snapshot':SOURCE.name,'baseline_snapshot':args.baseline_snapshot.name,
        ...
        'attention_mask_qualification_sha256':sha256(args.attention_mask_qualification) if args.attention_mask_qualification else None,
        'cases':cases,'global_sequences':batch,'board_size':9,'fma_flops':2,
        'scope':'Logical differentiated model graph with static scans expanded. Includes full encoder, helper/head, backward matrices, and explicit rematerialization. Splash counts complete active square tiles, including padding/masked work and score recomputation in the separate dQ/dKV kernels. Board-prefix block counts are independently checked against both sparse forward/dQ and dKV mask schedules.',
