...
def main():
    ...
    if c['kind']!='fixed_policy_learning' or c['evaluation']['run_test']:
        raise ValueError('Expected a single-policy run with test targets closed')
    ...
    if c['evaluation'].get('training_probe_games'):
        ...
        chosen=training_probe.select({b:by_bucket['expert',b] for b in buckets},c['evaluation']['training_probe_games'])
        ids=[entry for b in buckets for entry in chosen[b]]
        probe_positions=sum(int(data.shards[s]['expert_offsets'][e+1]-data.shards[s]['expert_offsets'][e]) for _,s,e in ids)
        expected_turns=[0]+[i for i in range(1,c['steps']+1) if i%c['eval_every']==0 or i==c['steps']]
        for report in reports:
            curve=report['training_probe_history']
            if [x['turn'] for x in curve]!=expected_turns:raise ValueError('Missing training-probe checkpoint')
            if any(x['episode_ids_sha256']!=digest(ids) or x['split']!=0 or x['metrics']['expert_count']!=probe_positions for x in curve):
                raise ValueError('Training-probe population differs')
            if curve!=reports[0]['training_probe_history']:raise ValueError('Training-probe metrics differ across ranks')
    import audit_population
    population_audit=audit_population.audit(data,c,reports)
    ...
    report={'kind':'single_policy_learning_audit','operator_snapshot':SOURCE.name,'status':'passed',
        ...
        'training_probe_curve':reports[0].get('training_probe_history',[]),
        'overfit_observations':reports[0].get('overfit_observations',[]),
