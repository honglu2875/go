...
def validate(c):
    ...
    if (not set(fields.split()) <= set(c) or set(c) - set(fields.split()) - {'fork'}
            or c['schema_version'] != 1 or c['kind'] != 'visual_causal_distillation'):
        raise ValueError('Unknown or missing visual learning fields')
    if 'fork' in c and (not isinstance(c['fork'], dict) or set(c['fork']) != {'candidate', 'candidate_sha256'}):
        raise ValueError('Explicit pinned fork candidate required')
    ...
    l = c['learner']
    required_learner = set('games_per_role learning_rate end_learning_rate warmup_steps beta1 beta2 epsilon weight_decay max_grad_norm loss_weights'.split())
    if not required_learner <= set(l) or set(l) - required_learner - {'augmentation'}:
        raise ValueError('Unknown optimizer field')
    if l.get('augmentation', 'none') not in ('none', 'd4'):
        raise ValueError('Unknown complete-history augmentation')
    ...
    e = c['exits']
    if set(e) != {'depths', 'loss_weight', 'temperature'} or e['depths'] != sorted(set(e['depths'])):
    ...
    if (e['loss_weight'] > 0 and not e['depths']) or not 0 <= e['loss_weight'] <= 10 or not .1 <= e['temperature'] <= 10:
