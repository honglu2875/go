...
def validate(c):
    ...
    if (not set(keys.split()) <= set(c) or set(c) - set(keys.split()) - {'suffix_replay'}
            or type(c.get('suffix_replay', False)) is not bool
            or c['schema_version'] != 1 or c['kind'] != 'visual_inference_service'
...
def qualify(runner, native, search, q, report):
    ...
    report['qualification'] = {'exact_leaf_predictions': exact_checks, 'maximum_absolute_error': max_error,
        ...
        'scope': 'Exact native MCTS leaves compared with full training-path outputs under the declared architecture and history contract.'}
...
def run(args, c, report):
    ...
    if c['candidate'] is not None:
        ...
        trained_recipe = checked['snapshot'] / checked['manifest']['recipe']
        for dependency in ('model.py', 'cnn.py', 'transformer.py', 'config.py', 'transformer_config.py'):
            if sha256(trained_recipe / dependency) != sha256(Path(__file__).with_name(dependency)):
                raise ValueError('Baseline inference dependency differs: ' + dependency)
    ...
    runner = inference.Runner(params, net, native, rules, slots=c['slots'], cache_positions=search['cache_positions'],
        network_version=version, exit_depth=c['exit_depth'], max_block=c['max_block'], suffix_replay=c.get('suffix_replay', False))
    report.update(host_rank=host, jax_rank=jax.process_index(), devices=[str(d) for d in jax.local_devices()],
                  ...
                  suffix_replay=c.get('suffix_replay', False))
    if c['mode'] == 'qualification':
        ...
        try:
            ...
            incomplete_matches = any(process.returncode != 0 for process, _, _, _ in jobs)
        finally:
        ...
        # A completed scheduled panel can contain unresolved move caps. Let
        # every host finish its panel before publishing that scientific
        # failure; genuine local exceptions still trigger prompt cancellation.
        mh.sync_global_devices('visual-matches-all-hosts-collected')
        if incomplete_matches:
            raise RuntimeError('Scheduled KataGo panel collected; one or more games incomplete')
    report['cache_stats'] = runner.stats; report['compilation'] = runner.compilation
