...
MODES=tuple(prefix+name for prefix in ('','dense_','cached_') for name in ('sequential','behavior','joint'))
...
def audit_segment(directory,row,c,model_id):
    ...
    mode=row['mode'];require(mode in MODES,'Unknown mode');base=mode.removeprefix('cached_').removeprefix('dense_');cached=mode.startswith(('cached_','dense_'))
    horizon=1 if base=='sequential' else c['horizon'];samples=c['samples']if base=='behavior'else 1
    target=row['target_moves_per_game'];remaining=[target]*c['games'];reconstructed=[[]for _ in events]
    counts=Counter(inactive_game_slots=0,cache_reset_games=0,cache_root_append_slots=0,retained_cache_output_bytes=0)
    previous=[None]*c['games'];action_count=c['actors']['size']**2+1
    for number,packet in enumerate(packets):
        ...
        if cached:require(len(packet['cache_controls'])==len(packet['cache_commits'])==c['games'],'Cache metadata coverage differs')
        else:require(packet['cache_controls']is None and packet['cache_commits']is None,'Uncached packet claims a device cache')
        for game,(ticket,resolution,allowance)in enumerate(zip(packet['tickets'],packet['resolutions'],packet['allowances'])):
            ...
            if ticket is None:
                ...
                if cached:require(packet['cache_controls'][game]==[0,1,0,action_count,0]+[-1]*horizon
                                  and packet['cache_commits'][game]is None,'Inactive cache controls/commit differ')
            ...
            if cached:
                prior=previous[game];reset=prior is None or prior[0]['episode']!=ticket['episode']
                if prior is not None:require(ticket['generation']==prior[0]['generation']+1,'Retained-cache ticket generation differs')
                if reset:
                    require(ticket['move_number']==0 and (prior is None or ticket['episode']==prior[0]['episode']+1),'Cache reset on a nonempty/unexpected episode')
                    expected=[1,1,0,action_count,0]+[-1]*horizon
                else:
                    pt,pr=prior;count=len(pr['actions']);position=pt['move_number']+count
                    require(position==ticket['move_number'],'Retained cache does not end at the current native prefix')
                    selector=((position-1)%2)*samples+pr['selected_samples'][-1]
                    expected=[1,0,selector,pr['actions'][-1],count]+pr['actions']+[-1]*(horizon-count)
                require(packet['cache_controls'][game]==expected,'Cache control differs from reconstructed committed history')
                selector=((ticket['move_number']+len(actions)-1)%2)*samples+resolution['selected_samples'][-1]
                certificate={'selector':selector,'valid_through':ticket['move_number']+len(actions)-1,
                    'committed_ply':ticket['move_number']+len(actions),'last_actual_action':actions[-1]}
                require(packet['cache_commits'][game]==certificate,'Retained branch certificate differs')
                previous[game]=(ticket,resolution);counts['cache_reset_games']+=int(reset)
    ...
    kv_tokens=2*samples*c['model']['max_tokens']if mode.startswith('dense_')else c['model']['max_tokens']+2*samples*horizon
    carry_bytes=games*(c['model']['blocks']*2*kv_tokens*c['model']['width']*4+2*actions*4)if cached else 0
    require(row['retained_cache_bytes']==carry_bytes,'Retained tensor dimensions differ')
    counts.update(dispatches=dispatches,real_moves=games*target,prefill_token_slots=0 if cached else dispatches*games*c['model']['max_tokens'],
        frame_array_bytes=dispatches*games*(4*c['model']['max_tokens']+8+actions+actions-1),
        input_array_bytes=dispatches*games*(4*(5+horizon if cached else c['model']['max_tokens'])+8+actions+actions-1),
        output_array_bytes=dispatches*games*horizon*(4*(2*samples*(1+actions)+actions)+2*samples*(actions-1)),
        cache_root_append_slots=dispatches*games if cached else 0,retained_cache_output_bytes=dispatches*carry_bytes,
    ...
    actual=row['counters'];timers=('native_start_and_frame_decode_seconds','inference_and_transfer_seconds','native_resolve_and_response_decode_seconds',
        'cache_prepare_seconds','cache_commit_seconds','cache_drain_seconds')
...
def audit_host(root,directory,protocol,host):
    ...
    require(result['kind']=='compact_kv_trace_throughput' and result['status']=='passed' and 'error'not in result
    ...
    require(result['candidate']==descriptor and result['candidate_sha256']==sha256(source/c['candidate'])
            and trained['model_code_sha256']==sha256(source/'research/recipes/trace_compact/model.py')
            and result['training_dtype']==trained['config']['model']['dtype']
            and result['execution_dtype']==c['model']['dtype']=='float32'
            and result['execution_matmul_precision']==c['matmul_precision']=='highest','Fixed execution model or numerical control differs')
    ...
    require([r['mode']for r in result['qualification']]==list(MODES),'Qualification modes differ')
...
def main():
    ...
    for repetition in range(len(c['orders'])):
        ...
        for mode in MODES[1:]:summary[mode]['speed_ratio']=summary['sequential']['slowest_host_seconds']/summary[mode]['slowest_host_seconds']
        repetitions.append(summary)
    ratios={mode:[r[mode]['speed_ratio']for r in repetitions]for mode in MODES[1:]}
    paired={mode:[r[mode.removeprefix('cached_')]['slowest_host_seconds']/r[mode]['slowest_host_seconds']for r in repetitions]for mode in MODES if mode.startswith('cached_')}
    vs_dense={mode:[r['dense_'+mode.removeprefix('cached_')]['slowest_host_seconds']/r[mode]['slowest_host_seconds']for r in repetitions]for mode in MODES if mode.startswith('cached_')}
    gate=protocol.get('primary_speed_criterion');met=None
    if gate is not None:
        values=[r[gate.get('baseline','sequential')]['slowest_host_seconds']/r[gate['mode']]['slowest_host_seconds']for r in repetitions]
        met=statistics.median(values)>=gate['minimum_median_ratio'] and min(values)>=gate['minimum_each_repetition_ratio']
    report={'schema_version':1,'kind':'compact_kv_trace_audit','status':'passed','analysis_snapshot':SOURCE.name,
        ...
        'cached_to_same_mode_ratios':paired,'median_cached_to_same_mode_ratios':{k:statistics.median(v)for k,v in paired.items()},
        'compact_to_dense_ratios':vs_dense,'median_compact_to_dense_ratios':{k:statistics.median(v)for k,v in vs_dense.items()},
