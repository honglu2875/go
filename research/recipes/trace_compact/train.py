...
MODES.update({prefix+name:value for prefix in ('dense_','cached_') for name,value in list(MODES.items())})
...
def validate(c):
    ...
    if c['games']*c['timed_moves_per_game']*len(MODES)*len(c['orders'])>2**22:
...
def play(native,execute,params,key,model_id,c,mode,target,output,label,synchronize=None,allocate_cache=None):
    ...
    horizon,samples,_,_=MODES[mode];cached=mode.startswith(('cached_','dense_'))
...
def run(args,c,report):
    ...
    from dense_decode import decode_cached as decode_dense,empty_cache as empty_dense_cache
    ...
    for name,(horizon,samples,coupling,oracle)in MODES.items():
        ...
        cached=name.startswith(('cached_','dense_'))
        if cached:
            cache_key=(name.split('_')[0],horizon,samples)
            if cache_key not in initial_carries:
                factory=(lambda:empty_dense_cache(c['model'],games,samples))if name.startswith('dense_')else (lambda:empty_cache(c['model'],games,samples,horizon=horizon))
                initial_carries[cache_key]=jax.jit(factory,out_shardings=batched)()
                jax.block_until_ready(initial_carries[cache_key])
            decoder=decode_dense if name.startswith('dense_')else decode_cached
            controls=np.full((games,5+horizon),-1,np.int32);controls[:,:5]=[1,1,0,actions,0]
            current_dummy=(params,initial_carries[cache_key],jax.device_put(controls,batched),*dummy[2:])
            compiled=jax.jit(lambda p,state,ct,l,e,k,s,legal:decoder(p,state,ct,l,e,k,s,c['model'],
    ...
    def execute(mode,p,t,l,e,k,s,legal,carry,control):
        ...
        if mode.startswith(('cached_','dense_')):
    ...
    def allocate_cache(mode):return initial_carries[(mode.split('_')[0],*MODES[mode][:2])]
...
def main():
    ...
    report={'schema_version':1,'kind':'compact_kv_trace_throughput','status':'running','snapshot_id':ROOT.name,
