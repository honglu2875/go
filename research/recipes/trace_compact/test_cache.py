...
from cached_decode import decode_cached,empty_cache,rebuild_cache,append_delta
...
class RetainedCacheTests(unittest.TestCase):
    ...
    def test_shared_attention_matches_full_prefix_at_heterogeneous_history_boundaries(self):
        c={**self.c,'blocks':2};p=model.initialize(817,c);games=3;branches=8
        lengths=jnp.asarray([0,5,9],jnp.int32)
        rng=np.random.default_rng(619);tokens=rng.integers(0,10,(games,13),dtype=np.int32);tokens[:,0]=10
        _,base=model.prefill(p,jnp.asarray(tokens),lengths,c)
        # Invalid future slots deliberately hold finite junk; only the masks
        # may decide whether a key/value contributes to attention.
        base=jax.tree.map(lambda x:jnp.where((jnp.arange(13)[None,:]<=lengths[:,None])[:,:,None,None],x,4096.),base)
        delta=jax.tree.map(lambda x:jnp.full_like(x,4096.),empty_cache(c,games,branches//2)['delta'])
        tapes=np.broadcast_to(tokens[:,None,:],(games,branches,13)).copy()
        compiled=jax.jit(lambda delta,t,depth:append_delta(p,base,delta,t,lengths,depth,c))
        for depth in range(1,4):
            chosen=rng.integers(0,10,(games,branches),dtype=np.int32)
            h,delta=compiled(delta,chosen,jnp.asarray(depth,jnp.int32))
            for game in range(games):tapes[game,:,int(lengths[game])+depth]=chosen[game]
            positions=jnp.repeat(lengths+depth,branches)
            states,reference=model.prefill(p,jnp.asarray(tapes.reshape(-1,13)),positions,c)
            np.testing.assert_allclose(np.asarray(h).reshape(-1,c['width']),
                np.asarray(states[jnp.arange(games*branches),positions]),rtol=2e-5,atol=2e-5)
            for actual,expected in zip(jax.tree.leaves(delta),jax.tree.leaves(reference)):
                np.testing.assert_allclose(np.asarray(actual[:,:,depth]).reshape(games*branches,c['heads'],-1),
                    np.asarray(expected[jnp.arange(games*branches),positions]),rtol=2e-5,atol=2e-5)
