...
def main():
    ...
    require(audit['kind']=='compact_kv_trace_audit' and audit['status']=='passed'
    ...
    require(duration>0 and abs(duration*1e-6-profile['run']['elapsed_seconds'])<.005
            and marker['args']['mode']==profile['run']['mode']=='cached_behavior'
    ...
    modes={}
    mode_names=tuple(prefix+name for prefix in ('','dense_','cached_')for name in ('sequential','behavior','joint'))
    compiler=[]
    for host,result in enumerate(hosts):
        values={};host_directory=attempt/f'rank-{host}/artifacts'
        for mode in mode_names:
            entry=result['compilation'][mode];cost_path=host_directory/(mode+'.cost.json');hlo_path=host_directory/(mode+'.hlo.txt.gz')
            require(sha256(cost_path)==entry['cost_sha256']and sha256(hlo_path)==entry['hlo_sha256'],'Compiler artifact changed')
            cost=read_json(cost_path)
            require(entry['compiler_estimates']=={k:cost[k]for k in ('flops','bytes accessed','transcendentals')if k in cost},'Recorded compiler estimates differ')
            hlo=gzip.decompress(hlo_path.read_bytes()).decode()
            collectives=Counter(re.findall(r'\b((?:all-gather|all-to-all|all-reduce|collective-permute|reduce-scatter|collective-broadcast)(?:-start|-done)?)\(',hlo))
            values[mode]={'estimates':entry['compiler_estimates'],'named_collective_occurrences_in_hlo':dict(collectives)}
        compiler.append({'host':host,'modes':values})
    for mode in mode_names:
        ...
        modes[mode]={'summed_host_loop_seconds':seconds,'summed_counters':dict(total),
            ...
            'cache_guard_and_drain_fraction':(total['cache_prepare_seconds']+total['cache_commit_seconds']+total['cache_drain_seconds'])/seconds,
            'active_history_fraction_of_prefill_slots':total['active_prefix_tokens']/total['prefill_token_slots']if total['prefill_token_slots']else None,
    ...
    report={'schema_version':1,'kind':'trace_packet_cost_and_occupancy_audit','status':'passed','analysis_snapshot':SOURCE.name,
        ...
        'compiler_estimates_by_host':compiler,
        ...
        'limitations':['Module timeline occupancy includes device computation, internal copies, barriers and waits; it is not MXU utilization or MFU.',
            'Only the cached behavior mode on host 0 was profiled. Profile overhead and its shorter prefix prevent a cross-mode utilization comparison.',
            ...
            'Active history slots describe canonical native frames. Cached modes perform zero full-prefill slots during timed loops.',
            'Compiler FLOPs and bytes accessed are estimates, without a normalized physical-chip or measured-HBM-traffic claim.',
            'Named-collective text counts do not rule out runtime communication, chip-internal barriers or operations hidden behind custom calls.']}
