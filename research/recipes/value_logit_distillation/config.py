...
def validate(c):
    ...
    if set(m) != set('size komi width heads blocks max_tokens dtype expert_temperature behavior_temperature board_width board_blocks board_mode value_objective'.split()):
        raise ValueError('Unknown model configuration')
    if m['value_objective'] not in ('mse','bce2'):
