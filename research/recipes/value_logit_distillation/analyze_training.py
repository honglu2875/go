#!/usr/bin/env python3
"""Audit fixed-budget MSE/BCE2 training, lineage, sampling and prediction gates."""
import argparse
import copy
import hashlib
import json
...
def audit_arm(root, protocol, mode):
    ...
    for host in range(4):
        ...
        require(result['status'] == 'passed' and result['training_complete'] and result['kind'] == 'board_causal_expert_behavior_distillation'
                and result['snapshot_id'] == source.name and result['turn'] == protocol['steps'] and result['world_size'] == 4
                and result['board_mode'] == 'exact' and result['value_objective'] == mode,
    ...
    first = results[0]
    require(first['parameter_count'] == protocol['parameter_count'], 'Registered parameter count differs')
    ...
    return {'mode': mode, 'attempt': directory.name, 'snapshot': source.name, 'descriptor_sha256': sha256(descriptor_path),
            ...
            'global_exposures': {k: first['counters'][k] for k in ('updates', 'expert_token_exposures', 'behavior_token_exposures')},
            ...
            'maximum_segment_seconds': max(r['elapsed_segment_seconds'] for r in results), 'raw_files_sha256': files}, config
...
def main():
    ...
    protocol = read_json(a.protocol); require(protocol['kind'] == 'value_logit_distillation_pilot', 'Wrong training protocol')
    result = {'schema_version': 1, 'kind': 'value_logit_training_audit', 'status': 'failed', 'analysis_snapshot': SOURCE.name,
    ...
    try:
        for name, field in (('student_calibration/result.json', 'calibration_audit_sha256'),
                            ('value_logit_distillation/cpu_result.json', 'cpu_qualification_sha256'),
                            ('value_logit_distillation/tpu_qualification_result.json', 'tpu_qualification_sha256')):
        ...
        require(protocol['training_order'] == ['mse', 'bce2'] and rows['mse']['finished_unix'] < rows['bce2']['started_unix'],
                'Registered sequential training order differs')
        expected = copy.deepcopy(configs['mse']); expected['model']['value_objective'] = 'bce2'
        require(expected == configs['bce2'] and configs['mse']['seed'] == protocol['seed']
                and configs['mse']['steps'] == protocol['steps'], 'Training changed more than the registered objective')
        for name in ('model.py', 'train.py', 'config.py'):
            path = 'research/recipes/value_logit_distillation/' + name
            require(sha256(root / '.gozero/snapshots' / rows['mse']['snapshot'] / path)
                    == sha256(root / '.gozero/snapshots' / rows['bce2']['snapshot'] / path), 'Scientific implementation differs between arms')
        for key in ('parameter_count', 'initial_parameter_elements_sha256', 'global_exposures', 'sampler_state_sha256_by_jax_rank'):
            require(rows['mse'][key] == rows['bce2'][key], 'Parameters, initialization or sampling differs')
        x = rows['mse']['final_validation']; y = rows['bce2']['final_validation']; gate = protocol['primary_prediction_criterion']
        metrics = {'value_validation_mse_ratio': y['value_loss'] / x['value_loss'], 'expert_validation_kl_ratio': y['expert_kl'] / x['expert_kl'],
                   ...
                   'behavior_validation_nll_increase': y['behavior_loss'] - x['behavior_loss']}
        passed = (metrics['value_validation_mse_ratio'] <= gate['value_validation_mse_ratio_at_most']
                  ...
                  and metrics['behavior_validation_nll_increase'] <= gate['behavior_validation_nll_increase_at_most'])
