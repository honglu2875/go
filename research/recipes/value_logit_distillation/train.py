#!/usr/bin/env python3
"""Cloneable, resumable board/history expert and observed-behavior distillation."""
import argparse
import hashlib
import json
import os
...
def train(args, c, report):
    ...
    report.update(jax_rank=rank, world_size=world, jax_version=jax.__version__, devices=[str(d) for d in devices],
                  ...
                  value_objective=c['model']['value_objective'])
...
def main():
    ...
    report = {'schema_version': 1, 'kind': 'board_causal_expert_behavior_distillation', 'snapshot_id': SOURCE.name,
