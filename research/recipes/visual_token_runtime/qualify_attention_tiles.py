...
def run(c,report):
    ...
    report.update(jax_rank=jax.process_index(),host_rank=int(os.environ['GOZERO_HOST_RANK']),
        kernel_source_sha256=sha256(Path(kernel_source.__file__)),jax_version=jax.__version__,cases=[])
    ...
    def attention(tile,length):
        if tile==0:
            return lambda q,k,v:causal.dense_attention(q,k,v,jnp.broadcast_to(jnp.arange(length),q.shape[:2]),jnp.full((q.shape[0],),length))
        extent=math.ceil(length/tile)*tile
        mask=splash.MultiHeadMask([splash.CausalMask((extent,extent)) for _ in range(12)])
