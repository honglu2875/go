...
append_move_reference=append_move


def append_move(p,cache,previous_action,spatial,globals,c,*,attention_positions,active=None,network_version=0):
    """Carry KV buffers through the layer loop and update only the new slots."""
    b,size,_,_=spatial.shape;spec=layout(size,c);stride=spec['stride'];capacity=cache['keys'].shape[2]
    extent=attention_positions*stride-1;kh=c['kv_heads'];dim=c['width']//c['heads']
    shape=(b,c['layers'],capacity,kh,dim)
    if capacity<stride or cache['keys'].shape!=shape or cache['values'].shape!=shape or cache['lengths'].shape!=(b,) or cache['valid'].shape!=(b,):
        raise ValueError('Invalid cache dimensions')
    if not 1<=attention_positions<=c['max_positions'] or extent>capacity:raise ValueError('Invalid attention extent')
    active=jnp.ones(b,bool) if active is None else active
    good=(cache['valid']&(cache['lengths']>=stride-1)&(cache['lengths']%stride==stride-1)
          &(previous_action>=0)&(previous_action<=size*size)&(cache['lengths']<=extent-stride)
    ...
    # Even rejected rows use distinct in-bounds slots, writing their old values.
    slots=jnp.where(good,cache['lengths'],0)[:,None]+jnp.arange(stride)[None,:]
    def layer(carry,data):
        x,keys,values=carry;block,index=data;q,k,v=project(x,block,positions,c)
        def update(buffer,new):
            old=buffer[rows,index,slots]
            value=jnp.where(write[:,None,None,None],new,old)
            return buffer.at[rows,index,slots].set(value,indices_are_sorted=True,unique_indices=True,mode='promise_in_bounds')
        keys,values=update(keys,k),update(values,v)
        k=jax.lax.dynamic_slice(keys,(0,index,0,0,0),(b,1,extent,kh,dim))[:,0]
        v=jax.lax.dynamic_slice(values,(0,index,0,0,0),(b,1,extent,kh,dim))[:,0]
        return (finish(x,dense_attention(q,k,v,positions,lengths),block,c),keys,values),None
    (x,keys,values),_=jax.lax.scan(layer,(x,cache['keys'],cache['values']),(blocks(p),jnp.arange(c['layers'])))
    logits=policy(p,x[:,-1],size,c)
    updated={**cache,'keys':keys,'values':values,'lengths':lengths,'valid':cache['valid']&(~active|good)}
    return jnp.where(write[:,None],logits,0.),updated
