...
This action-history model is a policy-only execution prototype. It has no
learned weights yet and does not replace the exact-state MCTS evaluator.
"""
import math
import jax
...
def losses(p,tokens,lengths,policy_targets,observed_moves,outcomes,valid,c):
    h,_=prefill(p,tokens,lengths,c);actions=c['size']**2+1
    counts=historical_counts(tokens,lengths,actions)
    context=jnp.take_along_axis(counts,(jnp.arange(tokens.shape[1])%2)[None,:,None,None],axis=2)[:,:,0]
    play,behavior,value=head_outputs(p,h,context)
    divisor=jnp.maximum(1.,valid.sum())
    expert=-jnp.sum(valid*jnp.sum(policy_targets*jax.nn.log_softmax(play),-1))/divisor
    predicted=jax.nn.one_hot(observed_moves,actions)
    observed=-jnp.sum(valid*jnp.sum(predicted*jax.nn.log_softmax(behavior),-1))/divisor
    value_loss=jnp.sum(valid*(value-outcomes)**2)/divisor
    return expert+observed+value_loss,{'play_loss':expert,'behavior_loss':observed,'value_loss':value_loss}
