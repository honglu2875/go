...
def total_metrics(prediction,b,axis_name=None,*,stratify=False):
    ...
    if stratify:
        weights=mask*b['family_weights'][:,None]
        totals.update(family_count=jnp.sum(weights),family_ce=jnp.sum(ce*weights),
                      family_target_entropy=jnp.sum(entropy*weights))
        for opponent in range(8):
            part=mask&(b['opponent'][:,None]==opponent);prefix=f'opponent_{opponent}'
...
def averages(totals):
    ...
    for prefix in ['family',*[f'opponent_{i}' for i in range(8)]]:
...
def losses(p,b,c,*,axis_name=None):
    if c['architecture']=='causal_visual_policy':
        if c.get('first_pass_aux_weight',0):
            import draft_model
            main,draft=draft_model.forward(p,b['spatial'],b['global_features'],b['actions'],b['counts'],c)
            m=averages(total_metrics(main,b,axis_name));aux=averages(total_metrics(draft,b,axis_name))
            weight=c['first_pass_aux_weight']
            loss=(1-weight)*m['expert_ce']+weight*aux['expert_ce']
            return loss,{**m,'draft_ce':aux['expert_ce'],'draft_kl':aux['expert_kl'],'draft_top1':aux['expert_top1'],
