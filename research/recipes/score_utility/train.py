...
def sgf(game):
    size = game['size']
    text = '(;GM[1]FF[4]CA[UTF-8]SZ[%d]KM[%s]RU[Tromp-Taylor]AP[gozero:score_utility]' % (size, game['komi'])
...
def main():
    ...
    report = {'schema_version': 1, 'kind': 'static_score_utility_training', 'status': 'running', 'snapshot_id': ROOT.name,
