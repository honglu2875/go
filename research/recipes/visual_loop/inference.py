...
class Runner:
    def __init__(self, params, c, native, rules, *, slots, cache_positions,
                 network_version, exit_depth=None, max_block=16):
    ...
    def score(self, requests):
        ...
        replayed = self.replay([item[0] for item in changed.values()])
        self.stats['replay_seconds'] += time.perf_counter() - start_replay
        for (slot, (tape, common, leaf_hash)), observations in zip(changed.items(), replayed):
            if (observation_sha256(observations[-1]) != leaf_hash
                    or observation_sha256(observations[common]) != self.hashes[slot][common]):
                raise ValueError('Replayed path and exact native pending board differ')
            changed[slot].append(observations)
        ...
        while changed:
            ...
            for slot, (tape, common, _, exact) in changed.items():
                n = min(horizon, len(tape) - common); counts[slot] = n
                obs[slot, :n] = exact[common + 1:common + n + 1]
            ...
            for slot in list(changed):
                tape, common, leaf_hash, exact = changed[slot]; n = int(counts[slot])
                self.predictions[slot][common + 1:] = [{k: v[slot, i].copy() for k, v in prediction.items()} for i in range(n)]
                self.hashes[slot][common + 1:] = [observation_sha256(o) for o in exact[common + 1:common + n + 1]]
