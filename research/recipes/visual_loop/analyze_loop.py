...
def main():
            ...
            for rep, record in enumerate(condition['repetitions']):
                ...
                if condition['horizon']:
                    ...
                    if (accepted != record['accepted_draft_moves'] or replacements != record['replacement_moves']
                            or moves != accepted + replacements or record['dispatches'] != 2 * record['iterations']
    ...
    result = {'schema_version': 1, 'kind': 'visual_continuous_policy_loop_audit', 'status': 'passed',
        ...
        'limitations': 'A bounded policy-sampling recurrence; this is not MCTS or full-game self-play. All retained-cache/replacement work is timed, initial prefill and compilation are excluded. Paths are different samples from approximately equivalent BF16 target policies and can end at different lengths. Three repetitions are a screening result, not a robust performance interval. Raw committed tapes and final states are independently audited; full-reference numerical comparisons are retained as per-rank metrics, and individual acceptance uniforms/probability packets were not persisted for independent draw replay. CPU coupling tests and frozen source implement that contract. No hardware MFU, engine strength or RL sample-efficiency claim.'}
