...
def run(c, output, report):
    ...
    for horizon in c['horizons']:
        fn, result = compile_fn(f'packet-{horizon}', lambda p, ca, r, s, k, left: loop.packet(p, ca, r, s, k, left, net,
            horizon=horizon, depth=depth, size=size, komi=rules['komi'], version=version), common,
            (P(), cache_specs, P('data'), P('data'), P('data'), P('data')),
        ...
        for horizon in order:
            ...
            while np.any(left > 0):
                ...
                if horizon == 0:
                    ...
                    rows, verified, predictions = fn(params, ca, prediction, current, keys, put(left))
                    ...
                    ca, prediction, current, repaired = repair(params, ca, prediction, current, rows, verified, predictions, put(accepted), put(replacement))
                    valid, passes, repaired = jax.device_get((current['valid'], current['passes'], repaired))
                    dispatches += 2; repairs += int(repaired.sum()); accepted_moves += int(accepted.sum())
...
def entry(args):
    ...
    if (set(c) != set(fields.split()) or c['schema_version'] != 1 or c['kind'] != 'visual_policy_loop_profile'
    ...
    report = {'schema_version': 1, 'kind': c['kind'], 'snapshot_id': SOURCE.name, 'status': 'running',
        ...
        'scope': 'Complete bounded policy-decoding recurrence from full held-out histories. Root-anchored shallow expert draft, full block verification, host maximal coupling, retained-cache rollback and replacement scoring. Includes every loop dispatch and final cache materialization; excludes initial prefill/compilation and separately records symmetric final native/full-model audits. Different valid policy samples can end at different lengths, so report committed moves and terminal counts, not equal-trace speed.'}
