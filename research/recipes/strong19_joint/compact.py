...
def forward(p,spatial,glob,actions,counts,c,*,chunk_frames,inner_rematerialize=None,with_hidden=False):
    ...
    return packed_forward(p,full_tokens,draft_tokens,actions,counts,spatial.shape[2],c,full_features,draft_features,with_hidden=with_hidden)
...
def packed_forward(p,full_tokens,draft_tokens,actions,counts,size,c,full_features,draft_features,*,with_hidden=False):
    ...
    if with_hidden:
        return dict(policy=main,aux_policy=draft,
