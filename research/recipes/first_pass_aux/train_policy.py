...
def run(args, c, report):
    ...
    common_indices=[i for i,item in enumerate(schema) if not item['path'].startswith(('head.local.','head.context.'))]
    ...
    def evaluate(split,*,draft=False):
        ...
        print(json.dumps({'kind': 'draft_heldout' if draft else 'visual_heldout', 'host': host, **result}), flush=True)
        return result
    def save():
        start = time.perf_counter()
