#!/usr/bin/env python3
"""Qualify state-validated paired execution of a fixed trained Go policy."""
import argparse
...
def validate(c):
    ...
    fields(m,'size komi width heads blocks max_tokens dtype expert_temperature behavior_temperature board_width board_blocks board_mode')
    if m['board_mode']!='exact' or not integer(m['board_width'],4,128) or not integer(m['board_blocks'],1,8):
        raise ValueError('Invalid exact-state encoder dimensions')
...
def run(args,c,report):
    ...
    if getattr(native,'STATE_TRACE_ABI_VERSION',None)!=1:raise ValueError('State trace ABI differs')
    ...
    if descriptor['kind']!='board_causal_history_policy':raise ValueError('Expected board-conditioned trained policy')
    ...
    dummy=(params,fake_tokens,np.zeros(c['games'],np.int32),np.zeros(c['games'],np.uint32),key,
           np.zeros((c['games'],c['actors']['size']**2),np.uint8),
           np.ones((c['games'],c['actors']['size']**2+1),np.bool_))
    executables={};compiles={}
    modes=[('sequential',1,1,'independent',False,'place'),
           ('behavior_independent',c['horizon'],c['samples'],'independent',False,'place'),
           ('behavior_untrained',c['horizon'],c['samples'],'independent',False,'place'),
           ('known_policy_independent',c['horizon'],c['samples'],'independent',True,'place'),
           ('known_policy_joint',c['horizon'],1,'shared',True,'place'),
           ('root_state_joint',c['horizon'],1,'shared',True,'hold')]
    for name,horizon,samples,coupling,oracle,update in modes:
        ...
        if name=='behavior_untrained':
            ...
            continue
        function=jax.jit(lambda p,t,l,e,k,s,legal:decode(p,t,l,e,k,s,c['model'],horizon=horizon,
            samples=samples,coupling=coupling,oracle_behavior=oracle,
            root_legal=legal if c['actors']['root_legal_mask'] else None,board_update=update))
    ...
    def play(name,targets=None):
        ...
        _,horizon,samples,_,_,_=next(m for m in modes if m[0]==name)
        ...
        for packet in range(limit):
            ...
            tickets_text,tokens,lengths,legal,stones=engine.start_with_states([model_id,model_id],[0,0],c['horizon'])
            ...
            start=time.perf_counter()
            outputs=jax.device_get(executables[name](parameters,tokens,lengths,episodes,key,stones,legal))
            counts['inference_and_transfer_seconds']+=time.perf_counter()-start
            counts['output_bytes']+=sum(a.nbytes for a in outputs)
            counts['input_array_bytes']+=sum(a.nbytes for a in (tokens,lengths,episodes,stones,legal))
            counts['predicted_state_output_bytes']+=outputs[3].nbytes
            ...
            start=time.perf_counter()
            resolved=json.loads(engine.resolve_with_states(tickets_text,*[np.ascontiguousarray(a) for a in outputs]))
...
def main():
    ...
    report={'schema_version':1,'kind':'state_validated_causal_policy_qualification','status':'running','snapshot_id':ROOT.name,
            ...
            'limitations':['Fixed MCTS-distilled student and same own-policy parameters for both players.',
                ...
                'Place-only board proposals omit captures and suicide; Rust checks every consumed own-policy input and stops on mismatch.',
                'Root-state control reuses the root board but applies the same exact input checks; it cannot silently use stale policy inputs.',
                'The reused board-conditioned student failed its registered external strength gate. This study qualifies execution only.',
