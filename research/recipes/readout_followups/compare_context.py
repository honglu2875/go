"""Verify a context-conditioned spatial-readout intervention on a fixed-data comparison."""
import argparse
import json
import re
...
def main():
    ...
    dim=rc.pop('policy_context_dim',0)
    if lc.pop('policy_context_dim',0)!=0 or dim!=64 or lc!=rc:
        raise ValueError('Expected context correction as the only model-config intervention')
    for k in ('peak_lr','end_to_peak_lr','position_exposures'):
        if left[k]!=right[k]:raise ValueError('Non-readout setting differs: '+k)
    if right['parameters']-left['parameters']!=dim*(rc['encoder_width']+rc['width']):raise ValueError('Unexpected extra parameters')
    ...
    for n in ('encoder.py','policy_optimizer.py','policy_model.py','policy_config.py','train.py','observation_attention.py','profile_causal.py'):
    ...
    if (reports[1]['initial_context_common_parameter_elements_sha256']!=reports[0]['initial_parameter_elements_sha256']
            or reports[1]['initial_context_query_zero'] is not True):raise ValueError('Initial common weights differ or correction is nonzero')
    ...
    out={'status':'passed','kind':'context_spatial_readout_contrast','operator_snapshot':SOURCE.name,
        ...
        'scope':'Same data, optimizer, initial common weights, encoder allocation, tokens and depth. One added history-conditioned spatial correction with the same policy target. One-seed screening does not establish playing strength.'}
