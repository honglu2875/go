...
def run(args, c, report):
    ...
    # spatial study provenance begin
    common_indices=[i for i,item in enumerate(schema) if not item['path'].startswith(('head.local.','head.context.','head.refine.'))]
    report['initial_shared_parameter_elements_sha256']=digest_arrays({f'p_{i:04d}':initial[f'p_{j:04d}'] for i,j in enumerate(common_indices)})
    report['initial_spatial_projection_zero']=all(np.all(initial[f'p_{i:04d}']==0) for i,item in enumerate(schema) if item['path'].startswith('head.local.'))
    context_indices=[i for i,item in enumerate(schema) if not item['path'].startswith('head.context.')]
    report['initial_context_common_parameter_elements_sha256']=digest_arrays({f'p_{i:04d}':initial[f'p_{j:04d}'] for i,j in enumerate(context_indices)})
    report['initial_context_query_zero']=all(np.all(initial[f'p_{i:04d}']==0) for i,item in enumerate(schema) if item['path']=='head.context.q.weight')
    without_refinement=[i for i,item in enumerate(schema) if not item['path'].startswith('head.refine.')]
    report['initial_without_refinement_parameter_elements_sha256']=digest_arrays({f'p_{i:04d}':initial[f'p_{j:04d}'] for i,j in enumerate(without_refinement)})
    report['initial_refinement_output_zero']=all(np.all(initial[f'p_{i:04d}']==0) for i,item in enumerate(schema) if item['path']=='head.refine.out.weight')
    # spatial study provenance end
    del initial
    if net.get('first_pass_aux_weight',0):
        import qualify_draft_runtime
        report['draft_kernel_qualification']=qualify_draft_runtime.run(net,buckets,mesh,replica)
    if c.get('profile_decode', False) and net['architecture']=='causal_visual_policy':
        ...
        if net.get('first_pass_aux_weight',0):
            ...
            draft_output=args.output/'draft_decode';draft_output.mkdir(exist_ok=False)
            report['draft_decode_profile']=profile_draft.run(params,data,net,global_batch,replica,world,mesh,draft_output)
    ...
    if args.resume is not None:
        ...
        # spatial study provenance begin
        report['initial_shared_parameter_elements_sha256']=saved.get('initial_shared_parameter_elements_sha256',report['initial_shared_parameter_elements_sha256'])
        # spatial study provenance end
    ...
    def evaluate(split,*,draft=False,entries_by_bucket=None):
        ...
        for bucket in buckets:
            entries = (entries_by_bucket[bucket] if entries_by_bucket is not None else
                       [entry for role in ('expert',) for entry in selected_buckets[role, bucket][:c['evaluation']['games_per_bucket']]])
            ...
            for begin in range(0, math.ceil(len(entries) / world), batch_size):
                ...
                cache_key=('draft',bucket) if draft else bucket
                if cache_key not in compiled_evaluations:
                    start_compile = time.perf_counter()
                    def local_metrics(p,b):
                        if draft:
                            import draft_model
                            main,cheap=draft_model.forward(p,b['spatial'],b['global_features'],b['actions'],b['counts'],net)
                            totals=model.total_metrics(cheap,b,axis_name='data',stratify=True)
                            full_probability=jax.nn.softmax(jnp.where(b['legal'],main,-1e9),axis=-1)
                            draft_probability=jax.nn.softmax(jnp.where(b['legal'],cheap,-1e9),axis=-1)
                            live=jnp.arange(b['actions'].shape[1])[None,:]<b['counts'][:,None]
                            overlap=jnp.sum(jnp.minimum(full_probability,draft_probability),axis=-1)
                            agreement=jnp.argmax(full_probability,-1)==jnp.argmax(draft_probability,-1)
                            return {**totals,'full_policy_overlap_sum':jax.lax.psum(jnp.sum(overlap*live),'data'),
                                    'full_argmax_agreement_sum':jax.lax.psum(jnp.sum(agreement*live),'data')}
                        return model.total_metrics(model.logits(p,b,net),b,axis_name='data',stratify=True)
                    fn = jax.shard_map(local_metrics,
                    ...
                    compiled_evaluations[cache_key] = jax.jit(fn).lower(params, batch).compile()
                    timing['compilation_seconds'] += time.perf_counter() - start_compile
                totals = {k: float(v) for k, v in replica(compiled_evaluations[cache_key](params, batch)).items()}
        ...
        if draft:
            result['metrics']['full_policy_overlap']=sums['full_policy_overlap_sum']/max(sums['expert_count'],1)
            result['metrics']['full_argmax_agreement']=sums['full_argmax_agreement_sum']/max(sums['expert_count'],1)
        timing['evaluation_seconds'] += time.perf_counter() - start
        kind = 'visual_training_probe' if entries_by_bucket is not None else 'draft_heldout' if draft else 'visual_heldout'
        with (args.output/'evaluations.jsonl').open('a') as stream:
            stream.write(json.dumps({'kind':kind,**result},sort_keys=True)+'\n');stream.flush()
        print(json.dumps({'kind': kind, 'host': host, **result}), flush=True)
        return result
    def save():
        start = time.perf_counter()
        ...
        # spatial study provenance begin
        saved['initial_shared_parameter_elements_sha256']=report['initial_shared_parameter_elements_sha256']
        saved['initial_context_common_parameter_elements_sha256']=report['initial_context_common_parameter_elements_sha256']
        # spatial study provenance end
        ...
        if host == 0 and c.get('checkpoint_temporary',False):
            ...
            path, identity, temporary_record = checkpoint_stage.write(path, state=saved, arrays=arrays, actors='{}',
                compress=False,minimum_free_bytes=64*(1<<30))
    ...
    if 'draft_reference' in c and not args.resume:
        import reference_draft
        fresh_params=params
        params,reference_receipt=reference_draft.load(c['draft_reference'],c,schema,host,replicated)
        reference_receipt=next(x for x in gather_json(reference_receipt) if x is not None)
        if report['initial_parameter_elements_sha256']!=reference_receipt['parent_initial_parameter_elements_sha256']:
            raise ValueError('Auxiliary initialization differs from the paired parent')
        parent_main=evaluate(1);parent_draft=evaluate(1,draft=True)
        expected=reference_receipt['historical_full_validation']
        if c['evaluation']['games_per_bucket']==10000:
            if parent_main['episode_ids_sha256']!=expected['episode_ids_sha256'] or abs(parent_main['metrics']['expert_kl']-expected['metrics']['expert_kl'])>1e-5:raise ValueError('Restored parent full-validation policy differs')
        import profile_draft
        reference_output=args.output/'reference_draft_decode';reference_output.mkdir(exist_ok=False)
        reference_profile=profile_draft.run(params,data,net,global_batch,replica,world,mesh,reference_output)
        report['draft_reference_evaluation']={**reference_receipt,'main_validation':parent_main,
            'draft_validation':parent_draft,'draft_decode_profile':reference_profile}
        params=fresh_params
    ...
    with (args.output / 'metrics.jsonl').open('x') as log:
        while turn < stop:
            ...
            if turn % c['eval_every'] == 0 or turn == c['steps']:
                ...
                if probe is not None:
                    training_history.append(evaluate(0,entries_by_bucket=probe))
                    paired_validation=([report['initial_validation']] if 'initial_validation' in report else [])+history
                    observation=training_probe.overfit_observation(paired_validation,training_history)
    ...
    if c.get('profile_decode', False) and net['architecture']=='causal_visual_policy':
        ...
        if net.get('first_pass_aux_weight',0):
            import profile_draft
            draft_output=args.output/'trained_draft_decode';draft_output.mkdir(exist_ok=False)
            report['trained_draft_decode_profile']=profile_draft.run(params,data,net,global_batch,replica,world,mesh,draft_output)
    if net.get('first_pass_aux_weight',0):
        report['draft_validation']=evaluate(1,draft=True)
