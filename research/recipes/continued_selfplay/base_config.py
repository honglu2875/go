...
def validate(c):
    ...
    fields(n, 'architecture heads mlp_ratio max_board_size width blocks groups value_hidden dtype score_utility_factor score_scale', 'model')
    if n['architecture'] not in ('cnn','attention') or not integer(n['heads'],1,64) or not integer(n['mlp_ratio'],1,16) or not integer(n['max_board_size'],1,26):
    ...
    if n['width']%n['heads']: raise ValueError('Attention heads must divide model width')
    ...
    if a['size']>n['max_board_size']: raise ValueError('Actor board exceeds model size limit')
