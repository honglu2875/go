...
from gozero import checkpoints, checkpoint_forks
...
def train(args, c, report):
    ...
    origin, fork_spec, native_path, receipt = checkpoint_forks.contract(ROOT.parents[2], ROOT, c)
    if args.native_receipt is not None and args.native_receipt.resolve() != native_path:
        raise ValueError('Native CLI override differs from the frozen dependency')
    args.native_receipt = native_path
    if len(set(gather_digest(hashlib.sha256(canonical_json(origin)).hexdigest()))) != 1:
        raise ValueError('Hosts selected different initialization contracts')
    phase_start_turn = 0 if fork_spec is None else fork_spec['parent_group']['turn']
    phase_start_counters = {k: 0 for k in ('real_moves','completed_games','truncated_games','eligible_rows','updates',
                                         'neural_batches','neural_slots','active_neural_evaluations')}
    if fork_spec is not None:
        entry = next(e for e in fork_spec['rank_checkpoints'] if e['jax_rank'] == rank)
        phase_start_counters = entry['counters']
    report['initialization'] = origin
    report['phase_start_turn'] = phase_start_turn
    ...
    if args.resume:
        ...
        if (state.get('initialization') != origin or group.get('initialization_sha256') != hashlib.sha256(canonical_json(origin)).hexdigest()
                or state.get('phase_start_turn') != phase_start_turn or state.get('phase_start_counters') != phase_start_counters
                or state['snapshot_id'] != ROOT.name or state['config_sha256'] != config_sha or state['jax_rank'] != rank
        ...
        loaded = (state, arrays, actor_state)
        report['resumed_from'] = {'path': str(args.resume), 'group_sha256': checkpoints.sha256(args.resume.with_suffix('.group.json')), 'turn': state['turn']}
    elif fork_spec is not None:
        loaded = checkpoint_forks.load_rank(ROOT.parents[2], fork_spec, jax_rank=rank, host_rank=report['host_rank'])
        report['forked_from'] = {'parent_snapshot': fork_spec['parent_snapshot'], 'parent_group_sha256': fork_spec['parent_group_sha256'],
                                 'turn': loaded[0]['turn']}
    if loaded is not None:
        state, arrays, actor_state = loaded
        if state['model_schema'] != schema:
            raise ValueError('Restored model schema differs')
    ...
    def save(engine, current_turn):
        ...
        state = {'schema_version': 1, 'snapshot_id': ROOT.name, 'config_sha256': config_sha,
                 ...
                 'initialization': origin, 'phase_start_turn': phase_start_turn, 'phase_start_counters': phase_start_counters}
        ...
        group = {'schema_version': 1, 'snapshot_id': ROOT.name, 'config_sha256': config_sha, 'world_size': world,
                 ...
                 'rank_manifests': rank_manifests, 'initialization_sha256': hashlib.sha256(canonical_json(origin)).hexdigest()}
    ...
    report.update(status='passed', turn=turn, config_sha256=config_sha, maximum_parameter_change=change,
                  phase_turns=turn-phase_start_turn,
                  phase_counters={k: value-phase_start_counters[k] for k,value in checkpoint_forks.work_counters(counters).items()},
...
def main():
    ...
    if args.native_receipt is not None: args.native_receipt = Path(args.native_receipt).resolve()
    ...
    report = {'schema_version': 1, 'kind': 'continued_selfplay_training', 'status': 'running', 'snapshot_id': ROOT.name,
