...
def validate(c):
    ...
    if not required|{'policy_spatial_bias'} <= set(c) or set(c)-required-{'policy_spatial_bias','policy_context_dim'} or c['architecture']!='causal_visual_policy':raise ValueError('Unknown shared-board model contract')
