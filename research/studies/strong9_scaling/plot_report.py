"""Render a closed comparison, reserving space for the mixture caption.

Presentation clone of plot_progress.py; the live pinned plotter is unchanged.
"""
import argparse
import csv
import json
...
def main():
    ...
    for i,(label,path) in enumerate(a.arm):
        ...
        if json.loads((attempt/'result.json').read_text())['status']!='passed':
            raise ValueError('Report requires closed successful arms')
