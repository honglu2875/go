"""Continue the audited objective repair to its original fixed training endpoint."""
import argparse
...
def main():
    ...
    prefix=ROOT/'runs'/p['prefix_attempt']
    for name,digest in p['prefix_evidence'].items():
        if sha(ROOT/name)!=digest:raise ValueError('Prefix evidence changed: '+name)
    prefix_result=json.loads((STUDY/'ce-prefix-001/result.json').read_text())
    if prefix_result['status']!='passed' or prefix_result['attempt']!=p['prefix_attempt']:raise ValueError('Prefix not retained and audited')
...
"""
        def preflight(host):
            ...
            reserve=(p['checkpoint_reserve_bytes'] if host in (0,p['replica_peer']) else 0) + (p['resume_staging_reserve_bytes'] if host!=0 else 0)
        ...
        log=command([ROOT/'.venv/bin/python','-B',snapshot/'ops/pod_run.py','--snapshot',snapshot,
            ...
            '--stop-after-turn',p['stop_turn'],'--resume-attempt',p['prefix_attempt'],'--resume-turn',p['prefix_turn']],'controller',p['timeout_seconds']+900)
        ...
        if closed['status']!='passed' or closed['snapshot_id']!=p['snapshot'] or closed['stop_after_turn']!=p['stop_turn'] or closed['resume_attempt']!=p['prefix_attempt']:
        ...
        command([p['python'],'-B',STUDY/'audit_continuation.py','--snapshot',snapshot,'--artifacts',
            *[attempt/f'rank-{i}/artifacts' for i in range(4)],'--purpose','learning','--stop-turn',p['stop_turn'],
            '--prefix-artifacts',*[prefix/f'rank-{i}/artifacts' for i in range(4)],'--output',folder/'audit.json'],'audit',600)
        ...
        result.update(kind='objective_only_joint_continuation',started=started,finished=time.time(),plan_sha256=args.plan_sha256)
