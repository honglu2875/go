"""Monitor one registered LR intervention and conditionally confirm its seed.
...
def confirmed_screen(result, registration, stage, registration_sha):
    if (result['kind'] != 'strong9_learning_rate_paired_contrast' or result['status'] != 'passed'
...
def inspect(path, expected):
    ...
    plan = read(path)
    if (plan['kind'] != 'strong9_lr_conditional_continuation' or plan['poll_seconds'] != 60
    ...
    launch_lr.inspect(registration, 'seed1')
...
class Continuation:
    ...
    def command(self, argv, label, timeout=900, accelerator=False):
        ...
        env = dict(os.environ, OPENBLAS_NUM_THREADS='1', OMP_NUM_THREADS='1',
                   PYTHONDONTWRITEBYTECODE='1', MPLCONFIGDIR='/tmp/gozero-mpl-lr15')
        if accelerator:
    ...
    def monitor(self, stage, attempt, final=False):
        ...
        self.command([ROOT/'.gozero/analysis-environments/plotting/bin/python','-B',
            ...
            '--arm','Transformer 1e-3',ROOT/'runs'/parent['attempt'],
            '--arm','Transformer 1.5e-3',attempt,
            ...
            if attempt is not None:
                ...
                if result_path.exists():
                    ...
                    if result['status'] != 'passed' or result['snapshot_id'] != self.registration['stages'][stage]['snapshot']:
                        raise RuntimeError('Registered LR attempt failed')
                ...
                if time.time() > launch['start_unix_time']+launch['timeout_seconds']+launch['prepare_timeout_seconds']+300:
                    raise TimeoutError('LR attempt did not close within declared bounds')
    ...
    def run(self):
        ...
        try:
            for stage in ('seed1', 'seed2'):
                if stage == 'seed2':
                    if not outcomes['seed1']['screen_passed']:
                        break
                    self.command([sys.executable,'-B',STUDY/'launch_lr.py','--plan',self.registration_path,
                        '--plan-sha256',self.registration['_sha256'],'--stage',stage],stage+'-launch',timeout=180,accelerator=True)
                attempt = self.wait(stage)
                self.command([sys.executable,'-B',STUDY/'finalize_lr.py','--plan',self.registration_path,
            ...
            outcome = dict(kind='strong9_lr_continuation_outcome',status='passed',created=time.time(),
