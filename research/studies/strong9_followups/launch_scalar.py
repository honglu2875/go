"""Launch one frozen scalar-only stage after the completed-study evidence gates."""
import argparse
...
def inspect(plan,stage):
    if plan['kind']!='strong9_scalar_intervention' or set(plan['stages'])!={'seed1','seed2'}:
        raise ValueError('Expected an explicitly paired scalar intervention')
    if sha(Path(__file__))!=plan['launch_operator_sha256'] or sha(Path(scalar_source.__file__))!=plan['source_helper_sha256']:
        raise ValueError('Scalar launch/source operator changed')
    if sha(STUDY/'scalar_compare.py')!=plan['comparison_operator_sha256']:raise ValueError('Comparison operator changed')
    ...
    prior=pinned(plan['preceding_outcome'])
    if prior['status']!='passed' or prior['kind'] not in ('strong9_lr_continuation_outcome','strong9_scalar_continuation_outcome'):
        raise ValueError('The preceding intervention has not completed its review')
    selection=pinned(plan['review'])
    if (selection['status']!='completed' or selection['mechanism']!=plan['mechanism']
            or selection['preceding_outcome_sha256']!=plan['preceding_outcome']['sha256']
            or selection['candidate_value']!=plan['candidate_value']):
        raise ValueError('Completed evidence review required')
    if sha(STUDY/'lr_compare.py')!=plan['paired_helper_sha256']:
        raise ValueError('Qualified paired comparison helper changed')
    rates=[];gates=[]
    for name,seed in (('seed1',91312427),('seed2',91312428)):
        ...
        scalar_source.identical_source(parent,candidate,plan['mechanism'])
        ...
        section,field=scalar_source.FIELDS[plan['mechanism']]
        rates.append(c[section][field])
    if len(set(rates))!=1 or rates[0]!=plan['candidate_value']:
        raise ValueError('Paired seeds must test the same scalar')
    ...
    manifest=scalar_source.verify(snapshot);c=read(snapshot/'resolved_config.json')
    qualified=dict(q['model']);model=dict(c['model'])
    if plan['mechanism']=='encoder_scale':
        qualified.pop('encoder_layer_scale');model.pop('encoder_layer_scale')
        prepared=pinned(plan['initialization_preparation'])
        cases=[r for r in prepared['cases'] if r['scale']==plan['candidate_value']]
        if prepared['status']!='passed' or len(cases)!=1 or not all(cases[0][k] for k in
                ('full_size_schema_equal','complete_decoding_budget_equal','non_scale_parameter_arrays_exact','small_forward_backward_stablehlo_identical')):
            raise ValueError('Encoder initialization preparation missing')
        for name,digest in prepared['source_files'].items():
            if sha(snapshot/manifest['recipe']/name)!=digest:raise ValueError('Prepared initialization source changed')
    if qualified!=model:raise ValueError('Qualified model changed beyond initialization')
    ...
    if stage=='seed2':
        ...
        if (first['kind']!='strong9_scalar_paired_contrast' or first['status']!='passed'
...
def main():
    ...
    if sha(a.plan)!=a.plan_sha256:raise ValueError('Scalar registration changed')
    ...
    with (STUDY/'.scalar-launch.lock').open('a+b') as lock:
        ...
        result=dict(kind='strong9_scalar_stage_process',stage=a.stage,trial=plan['trial'],pid=process.pid,
