"""Audit, replicate and contrast one completed registered LR intervention."""
import argparse
import json
import os
...
def main():
    ...
    if sha(a.plan)!=a.plan_sha256:raise ValueError('LR plan changed')
    plan=read(a.plan)
    if plan['kind']!='strong9_learning_rate_intervention' or sha(Path(__file__))!=plan['finalize_operator_sha256']:
        raise ValueError('Wrong finalization plan/operator')
    if sha(STUDY/'lr_compare.py')!=plan['comparison_operator_sha256'] or sha(Path(lr_source.__file__))!=plan['source_helper_sha256']:
    ...
    manifest=lr_source.verify(source)
    ...
    if not contrast.exists():
        invoke([STUDY/'lr_compare.py','--plan',a.plan,'--plan-sha256',a.plan_sha256,'--stage',a.stage,
    ...
    if result['status']!='passed' or result['plan_sha256']!=a.plan_sha256 or result['candidate']['snapshot']!=source.name:
        raise ValueError('Wrong LR contrast')
