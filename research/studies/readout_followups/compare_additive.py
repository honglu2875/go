"""Compare an audited additive readout with its prospectively pinned bilinear parent."""
import argparse
import hashlib
import json
...
def main():
    ...
    if reg['intervention']!='additive_nonlinear_readout_rank64':
    ...
    for label, audit, audit_path in zip(('Bilinear rank 64 parent', 'Additive rank 64'), audits, (reference_path,a.candidate_audit)):
    ...
    if right_model.pop('policy_readout_kind') != 'additive' or left_model != right_model:
    ...
    out = dict(status='passed', kind='paired_additive_readout_contrast', created_unix=time.time(), seed=seed,
