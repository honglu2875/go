"""Compare an audited rank-128 run with its prospectively pinned rank-64 seed."""
import argparse
import hashlib
import json
...
def main():
    ...
    for path, digest in ((a.registration, a.registration_sha256), (a.amendment, a.amendment_sha256),
                         (a.candidate_audit, a.candidate_audit_sha256)):
        if sha(path) != digest:
            raise ValueError('Pinned input changed: '+str(path))
    reg, amendment, candidate = map(read, (a.registration, a.amendment, a.candidate_audit))
    if amendment['parent_registration_sha256'] != a.registration_sha256:
        raise ValueError('Wrong amendment parent')
    ...
    if candidate_source.name != amendment['snapshots'][stage]:
    ...
    for label, audit, audit_path in zip(('Rank 64 parent', 'Rank 128'), audits, (reference_path,a.candidate_audit)):
    ...
    if left_model.pop('policy_context_dim') != 64 or right_model.pop('policy_context_dim') != 128 or left_model != right_model:
        raise ValueError('Unexpected model intervention')
    if reports[0]['initial_context_common_parameter_elements_sha256'] != reports[1]['initial_context_common_parameter_elements_sha256']:
    ...
    out = dict(status='passed', kind='paired_readout_rank_contrast', created_unix=time.time(), seed=seed,
        registration_sha256=a.registration_sha256, amendment_sha256=a.amendment_sha256,
