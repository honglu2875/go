"""Run the two frozen trained-model RPC/GTP CPU qualifications sequentially."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    out=STUDY/('rpc-cpu-'+case);out.mkdir(exist_ok=False)
    result_path=STUDY/('rpc-cpu-result-'+case+'.json')
    if result_path.exists():raise FileExistsError('Qualification result exists')
    started=time.monotonic();report=dict(kind='trained_joint_gtp_cpu_pair',status='running',created=time.time(),plan_sha256=a.sha256,models={})
    try:
        for arm in ('cnn','transformer'):
            ...
            report['models'][arm]=dict(path=str(path.relative_to(ROOT)),sha256=sha(path),
                                      client_searches=sum(len(row['searches']) for row in record['clients']),comparisons=record['comparisons'],
        ...
        report['scope']='Small trained19 RPC/GTP subprocesses, independent native searches and full JAX prediction checks. No KataGo opponent, TPU qualification or strength result.'
