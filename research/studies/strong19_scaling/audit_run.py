...
def audit(snapshot, directories, purpose):
    import numpy as np
    ...
    world = c['expected_processes']; horizon = c['steps']
    ...
    for directory in directories:
        ...
        require(read_json(directory / 'resolved_config.json') == c and r['config_sha256'] == digest
                ...
                and r['status'] == 'passed' and r['training_complete'] and r['turn'] == horizon
    ...
    return dict(kind='complete_joint_learner_audit', status='passed', snapshot=snapshot.name, purpose=purpose,
...
def main():
    ...
    result = audit(args.snapshot.resolve(), args.artifacts, args.purpose)
