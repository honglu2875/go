#!/usr/bin/env python3
"""Run one registered fixed-checkpoint expert-architecture evaluation arm."""
import argparse
import json
...
def main():
    ...
    p.add_argument('--expected-protocol-sha256', required=True); p.add_argument('--arm', choices=('history', 'state'), required=True)
    ...
    require(protocol['kind'] == 'state_expert_distillation_pilot' and protocol['maximum_eval_attempts_per_arm'] == 1, 'Study identity differs')
    ...
    require(descriptor['training_snapshot'] == arm['snapshot_id'] and trained['config']['model']['architecture'] == a.arm and trained['config']['model']['value_objective'] == 'mse'
    ...
    report = {'schema_version': 1, 'kind': 'state_expert_katago_suite', 'status': 'running', 'snapshot_id': SOURCE.name,
    ...
    try:
        for item in arm['evaluation_panels']:
            ...
            print(json.dumps({'kind': 'state_expert_panel_start', 'arm': a.arm, 'panel': item['id']}), flush=True)
